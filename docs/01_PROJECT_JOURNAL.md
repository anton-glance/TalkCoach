...
**Format:** Architect + product-owner + agent session. Single spike (#18) scoped, planned, implemented across one Phase-1/Phase-2/Phase-3 cycle plus four in-session corrections, and certified Gates 0–3 PASS / Gate 4 abandoned-by-design. Close-out reconciles the engine reversal across all docs. No production code committed (spike lives in throwaway `ParakeetRustSpike/`); documentation tarball only.

### Why the engine flipped (whisper → Parakeet)

Session 033 locked Architecture Z (whisper.cpp + Silero VAD) as the least-bad survivor of a four-spike latency tournament run under a 200ms first-token budget. That budget was then retired ("whatever-best-possible"). Critically, **no spike ever scored Parakeet for transcript word-fidelity** — the FluidAudio Parakeet spikes (#17.1, #17.1.5) died on latency/architecture (15s batch API; ~2s encoder cache warmup) before anyone measured whether the words were right. The whisper choice was by elimination, not by a head-to-head fidelity comparison.

The product owner's independent reference work (5 reads through Parakeet) found the distinction that mattered: **whisper transcribes _meaning_ and silently drops/changes words; Parakeet keeps the actual words — fillers, numbers, structure, disfluencies.** For a WPM product, word-count fidelity IS the product. That reframed the question from "which engine hits a latency budget" to "which engine counts words correctly," and reopened Parakeet — this time via `transcribe-rs`/`parakeet-rs` (Rust, ONNX, CPU/int8), NOT the FluidAudio Swift path that #17.1/#17.1.5 disproved. The CPU/int8 path is also the strategic foundation for the planned Windows port (no Metal dependency).

### Spike #18 — the word-fidelity-and-CPU-cost spike

Framed explicitly as NOT a streaming spike (Q3: the WPM number is a ~10s sliding average refreshed every ~3s; talk-vs-silence detection is already solved separately — so repeated short-batch is the target architecture, not token-streaming) and NOT a latency-budget spike (S033 retired the budget; latency is a measurement here, not a gate). The kill question: does Parakeet on CPU produce shippable word-counts on a rolling short window?

**Crate research (web):** the "WhisperKey/Handy fork" in the product owner's memory was imprecise. Two real crates exist: `transcribe-rs` (cjpais, multi-engine) and `parakeet-rs` (altunenes, Parakeet-only). Chose `parakeet-rs` v0.3.5 for smallest integration surface. It ships four model paths — CTC (EN batch), TDT (multilingual batch), EOU (streaming — FORBIDDEN, it's the same 120M model Spike #17.1.5 already killed on cache warmup), Nemotron (cache-aware streaming). Spine: **TDT v3 int8** (the exact model #17.1 proved loads clean on macOS; multilingual; one model across all gates).

**Ground truth:** the EN WPMSpike clips (en_slow 122w/67.2s/108.9wpm, en_normal 237w/103.9s/136.8wpm, en_fast 237w/69.7s/204.0wpm) have verbatim `.txt` + authoritative JSON `totalWords` — already in the repo, already verified. RU clips are count-only (their `.txt` is truncated/unreliable; JSON counts trusted: ru_slow 121, ru_normal 167, ru_fast 194).

**Gate results (production cell: 10s window / 3s hop):**

- **Gate 0 — build/load/decode: PASS.** `parakeet-rs` 0.3.5, ORT 2.0.0-rc.12 linked with NO manual `ORT_DYLIB_PATH` (Risk #1, expected HIGH, did not materialize — the arm64-darwin dylib ships in `ort-sys`). TDT int8 loaded in 964ms. Per-token timestamps PRESENT (real, monotonic) → Gate 2 flicker uses the detrended-overlap (Branch A) method. Per-token **confidence NOT exposed** — `TimedToken` has text/start/end only in v0.3.5.

- **Gate 1 — word-fidelity kill gate: PASS (with constraint).** en_slow 0.0% (122/122), en_normal 3.8% — both verbatim-faithful, keeping "you know"/"kind of"/conversational structure (the exact thing whisper failed). en_fast initially "failed" at 41.4% — but diagnosed (DIAG 1/2/3) as **full-buffer truncation, not inaccuracy**: the engine received the whole 69.7s buffer but stopped emitting at 37.85s (54% coverage). Chunked into 15s segments it recovered to 236/237 = 0.4%. Truncation is an artifact of a code path production won't use (full-buffer on a long clip), not an engine quality limit.

- **Gate 2 — rolling-window accuracy + cost sweep: PASS, all dimensions.** **Cost / Risk #3 (estimated >50% to fail): resolved decisively** — every cell hr_min > 1.0 (worst 2.818), per-batch 146–549ms vs 2–3s cadence; CPU ~232% mean / ~300% peak in bursts (≈2.5 cores, idle between). int8 Parakeet on CPU runs 3–19× faster than cadence. The first accuracy metric (clip-average expected count) was caught by the architect as INVALID — it measured speaker rate-variation, not engine error — and re-scored as (a) non-overlapping tile reconciliation vs ground truth and (b) engine self-consistency on shared overlap span (speaker variation cancels). Re-scored: en_fast 10s = 3.8%, 15s = 0.4%; **6s disqualified** (dense speech trips the truncation → 23% loss, ~1.5wps self-inconsistency = FM2 jitter risk). Pause stress test (en_normal 4s pause): window returns 2 words, no hallucination spike — FM2 clean.

- **Gate 3 — acoustic + multilingual robustness: PASS.** EN reconciled exactly to Gate 2 (en_fast 228/3.8%, en_normal 244/3.0% over 11 windows / 108s — first confirmation accuracy holds over a longer multi-window session). **RU count-only vs JSON: ru_slow 0.8%, ru_normal 1.8%, ru_fast 8.8% — all within ±15%.** The multilingual TDT spine counts Russian words accurately, validating the TDT-over-CTC choice. RU also confirms the token-ceiling is token-count-based across languages (longer Russian words = fewer tokens/sec = easier on the ceiling; no truncation at 10s).

- **Gate 4 — Nemotron cache-aware streaming probe: ABANDONED BY DESIGN.** Not blocked, not deferred-pending-slug — dropped after a UX analysis. Nemotron buys "continuous WPM update instead of 3s-stepped," which the product owner specced as unnecessary (Q3) and which a mockup test showed is _distracting_ (a number sliding every second fights FM1 glanceability). It is also EN-only and a different model, so adopting it would cost the multilingual coverage just validated. Negative expected value for v1. Recorded as a future-version option, not pursued.

### THE load-bearing finding: per-inference token ceiling

`parakeet-rs` TDT int8 has a **per-inference token ceiling, not a duration limit.** Below ~10s of _dense_ speech (and on full-buffer inference of long dense clips), it truncates mid-buffer and stops emitting. At ≥10s windows the dense content clears the ceiling and accuracy/self-consistency are excellent on both languages. **Production constraints (measured, not assumed):** (1) use a ≥10s rolling window — 6s is disqualified; (2) NEVER call full-buffer inference on a long clip; (3) no per-token confidence available in v0.3.5 (fine for WPM; a real constraint for the v1.x smart-filler work, which will need a different surface/crate).

### In-session corrections (failures documented per close-out instruction)

1. **fp32-vs-int8 download confusion.** The deprecated `huggingface-cli` no longer works (now `hf`); the new CLI treats post-repo args as positional filenames and ignored `--include`. First download pulled an incomplete fp32 set (2.4GB `.onnx.data` but missing the `encoder-model.onnx` graph file). `--dry-run` revealed the repo ships both fp32 and int8 variants. Corrected to int8 (`encoder-model.int8.onnx` self-contained + `decoder_joint-model.int8.onnx` + `nemo128.onnx` sentinel + `vocab.txt`).
2. **Binary hardcoded fp32 filenames** (spec was written before repo layout was known). Fixed: precondition check now expects the int8 names AND prints a diagnostic expected-vs-found listing on every run — a durable fix so filename mismatches are diagnosable in one run. Also surfaced that `parakeet-rs` does mel feature-extraction in Rust internally; `nemo128.onnx` is a download-completeness sentinel, never loaded.
3. **`groundTruthWPM` serde field bug.** Struct expected `groundTruthWpm`; the JSON uses `groundTruthWPM` (camelCase rename doesn't cover the `WPM` run). Fixed with explicit `#[serde(rename)]`.
4. **Gate 2 accuracy metric invalid** (clip-average expected-count confounds speaker rate-variation with engine error). Architect-caught; re-scored to tile-reconciliation + same-span self-consistency.
5. **Remainder-tile bug in TWO code paths.** The `while t + ws <= dur_s` tile loop dropped the final partial tile (31 words on en_fast = the 236-vs-205 discrepancy). Fixed in Gate 2, then found to ALSO exist in Gate 3 (architect caught the Gate-2/Gate-3 disagreement on the same clip). Resolved durably: extracted a single shared `transcribe_tiles()` function with exactly three call sites, so the bug cannot diverge between gates again.

### Process lessons

1. **Specs written ahead of ground-truth discovery encode assumptions that become hardcoded bugs.** Two name-mismatches this session (model filenames, JSON field) and a tiler-boundary bug all traced to code written against assumed shapes. Mitigations applied: diagnostic expected-vs-found precondition output; a single shared tiler. If a third name-mismatch class appears, add startup validation against actual file shapes.
2. **"Fix in one path, certify, discover the same bug in the sibling path" (the remainder-tile bug) is the same class as Session 032's "behavior correct under design A, wrong under design B."** Lesson reinforced: when fixing a bug, grep for every code path with the same pattern before certifying. The shared-function refactor is the structural prevention.
3. **A "PASS" label is not a certification.** Gate 2's first run printed "PASS" while its headline accuracy metric was measuring the wrong thing. The architect-side audit (does the metric answer the product question?) caught it. Green labels and green metrics are not the same as correct metrics.
4. **NO-SKIPPING held under pressure four times** (en_fast truncation diagnosis, accuracy-metric correction, remainder-tile reconciliation, Gate-3 RU conversion) — each STOP produced a cleaner result than pressing on would have. The discipline that produced S033's algorithmic pivot is now habitual.

### Time accounting

- Session start: 2026-05-22 13:44 UTC (bootstrap `date` call, first message establishing session)
- Session end: 2026-05-22 19:46 UTC (close-out write time)
- Wall-clock total: **~6.0h** (architect + product-owner + agent, single continuous session)
- Breakdown (estimate): spike scoping + strawman iteration ~1.0h; crate research ~0.3h; Phase 1 plan + 2 revisions + audit ~1.0h; Phase 2 impl + 4 corrections (agent, interleaved) ~1.5h; Gate 0–3 execution + architect audits + re-runs ~1.7h; close-out docs ~0.5h
- Variance: Spike #18 was un-estimated (opened and closed same session). Contrast with M3.7's ~60–83h: a single well-scoped spike with tight architect audit and NO-SKIPPING closed in one afternoon what the four-spike search could not in seven weeks — because the question was reframed (word-fidelity, not latency) and the ground truth already existed.

### State at close

Spike #18 CLOSED: Gates 0–3 PASS, Gate 4 abandoned-by-design. **Engine LOCKED: Parakeet TDT v3 int8 via `parakeet-rs` v0.3.5, CPU, 10s rolling window / 3s hop (cadence subject to UX tuning in integration — product owner: 3s steps feel distracting on the mockup, exact refresh TBD on real widget, definitely not per-second).** Architecture Z (whisper.cpp + Silero) superseded by Architecture AA before it shipped. Next: **M3.7.4 rewritten** — Parakeet `parakeet-rs` integration into the production transcriber stack, replacing the never-shipped whisper.cpp plan. The spike's `ParakeetRustSpike/` artifacts (CLI, results CSVs) remain on the local Mac for reference; not committed (throwaway spike dir).

---

## Session 033 — 2026-05-21 — M3.7 closes after seven-week STT engine search across four spikes (Apple SpeechAnalyzer batching gap → Parakeet TDT v3 SlidingWindow 18s batch → Parakeet 120M EOU 2.3s cache warmup → whisper.cpp 30s mel encoder floor) — every general-purpose ASR engine designed before 2024 has an architectural latency floor above 200ms; product owner relaxes C4 budget to "whatever-best-possible" pending 14-step smoke gate UX validation; close-out consolidates all spike learnings into journal + architecture + backlog + spikes docs to prevent future re-treading

**Format:** Architect-side close-out session. Architectural-decision-archive session, not implementation. Wraps M3.7 widget close-out (fix #6, fix #7) AND the four-spike STT engine search (#16, #17.1, #17.1.5, #17.2, #17.3) into a single set of doc updates so the next session can begin whisper.cpp integration with full prior-art visibility. No code commits; documentation tarball only.

### What happened

Session 033 spanned three weeks of calendar time across multiple sub-sessions. The narrative compresses to: M3.7 fix #6 implemented the widget-lifecycle spec from Session 032 (engine-ready trigger, 3s+2s linger, Phase G in-place session-switch). Smoke gate revealed Bug B-prime — `panel?.alphaValue = X` inside `NSAnimationContext.runAnimationGroup` snaps instead of animating. Fix landed (`animator().alphaValue = X` is the correct pattern). Three rounds of fixes ultimately produced the 7-state widget model (idle / warming / counting / waiting / wrapping / recovering / dismissed) with structured `widget-state: <ISO8601> <prev>→<next> reason=<reason>` log emission, commit `7670867` at the end of fix #7.

**Smoke gate against Voice Memos confirmed the widget UX works on real hardware.** Fade-out, 50% translucency on `.waiting`, linger, X-button dismiss, Phase G session-switch all validated. The state-transition CSV pivot (`tools/state-timeline.py`) became the canonical empirical-validation tool.

**The CSV pivot also surfaced the architectural problem that drove the rest of Session 033.** Apple SpeechAnalyzer's `tokenStream` does not emit tokens in real time — tokens arrive in 2-4 second bursts at SpeechAnalyzer's internal commit boundaries. The widget's `isInTokenSilence` state machine subscribed to this stream and consequently flipped to `.waiting` while the user was actively speaking. Smoke evidence (10s continuous speech → 13 hypothesis tokens in 23ms, then 4s gap with zero tokens, then more hypothesis revisions, then 31 isFinal=true tokens delivered in 11ms at the very end) made it clear: **Apple's token stream is a transcription-hypothesis-revision stream batched at commit boundaries, not a speaking-activity stream.**

The widget UX problem could not be fixed by tuning. It required a different signal: real-time voice activity from the audio buffer stream BEFORE transcription. That decision opened the seven-week, four-spike engine search.

### Four-spike STT engine search — narrative compressed

**Spike #16 — RMS-based VAD (FAIL).** Hypothesis: a simple energy-based VAD on raw audio buffers would deliver real-time voice activity. Built `RMSVoiceActivityDetector` with adaptive noise floor + hysteresis. Tested against 10 fixtures Anton recorded (5 scenarios × 2 mics — AirPods Pro + MacBook built-in array). 27-combination parameter sweep across hysteresis counts and threshold margins. **All combinations FAIL.** Best `speech_fn_max` of 16.09% still 3× over the 5% budget. Verdict: **far-field MacBook built-in mic puts speech only 5-10 dB above the noise floor — an information-content problem, not a tuning problem.** Energy alone cannot distinguish speech-after-pause from background HVAC. Forward note: any future spike using energy-only VAD on far-field mic will fail the same way. Use neural VAD (Silero) instead.

**Spike #17.1 — Parakeet TDT v3 via FluidAudio `SlidingWindowAsrManager` (FAIL).** Hypothesis from Anton's prior TiltTalk work: Parakeet TDT v3 is multilingual (25 European languages), sub-200ms latency on iOS, FluidAudio Swift package exists. Test on macOS. **Build clean (Risk A neutralized). Model load 119ms. Memory 78.9 MB. No hallucinations. Confidence scores exposed per-token (TiltTalk's wrapper hid this — they ARE there).** Three FluidAudio bugs filed (`padAudioIfNeeded` truncation, `finish()` continuation closure, default config incompatible with compiled model shape). **First-update latency 18424ms (vs 200ms budget, 92× over).** Root cause: the CoreML Preprocessor model was compiled with fixed input shape [1, 240000] = 15s; FluidAudio's default config produces 432000-sample windows that exceed this shape → CoreML rejects. Workaround forces 15-second batch mode. **Verdict: SlidingWindowAsrManager is a batch architecture, not a streaming one. The model and API choice matter — Parakeet works on macOS, but this API surface forces batching.** Forward note: confidence score IS exposed in FluidAudio's TranscriptionUpdate; TiltTalk's wrapper discarded it.

**Spike #17.1.5 — Parakeet 120M EOU via `StreamingEouAsrManager` (FAIL).** Different FluidAudio API, different model (120M parameters vs 600M). Designed for streaming with 160ms / 320ms / 1280ms chunk sizes. **GATE 1 FAIL at the 160ms smoke test alone (NO-SKIPPING early abort).** First-update latency 2336ms warm, 5831ms cold. Root cause: **loopback conformer encoder caches initialize to zeros, need ~29 inference steps × 80ms shift = 2320ms audio context before producing usable encoder features.** Cache warmup is constant ~2s regardless of chunk size. Architecturally untunable. **Verdict: stateful streaming encoders with zero-initialized caches cannot meet sub-200ms first-token latency on any chunk size — the warmup IS the architecture.** Forward note: the FluidAudio v0.14.7 API has TranscriptionUpdate exposing `text` and `isConfirmed: Bool` only — RNNT streaming decoder does NOT expose per-token confidence (sentinel -1.0); only utterance-level isConfirmed at EOU boundaries. This is a genuine capability gap from the SlidingWindow surface, not a bug.

**Spike #17.2 — whisper.cpp v1.8.4 + Silero VAD + whisper-small (FAIL, but tunable-looking).** Hypothesis: stateless-per-segment architecture with VAD-gated inference avoids encoder cache warmup. Built CMake + Metal + Swift-C bridging. **BUILD-GATE PASS, SMOKE-GATE PASS.** First-token latency 333-440ms across fixtures — 6-10× improvement vs #17.1 / #17.1.5, but 1.67-2.2× over 200ms budget. Hypothesis at the time: reduce `kLengthMs` from 3000ms → 1000ms, predicted C4 of 120-160ms based on assumed linear scaling. Other criteria PASS cleanly: C9 confidence (per-token log_prob median 0.745), C10 cafe noise, C11 memory 814 MB (within 1 GB spike budget). C6 hallucinations FAILed (32 ghost tokens on silence-only fixtures — Silero threshold 0.5 over-detects room tone + whisper-small hallucinates `[BLANK_AUDIO]` / "Thank you" on near-silence). C8 silence-boundary detection 0/6 — root cause: methodology bug, `emission_ms` is wall-clock, manifest events are audio-time, ~40s offset due to Metal JIT compile time. C12 SKIP because `real_world_test.caf` was at wrong path (Spike17.1.5_StreamingEOU/recordings/, not Spike16_BufferLevelVAD/recordings/). **Verdict: EARLY-GATE FAIL, parameter-tunable hypothesis. Forward note: macOS 26 beta has intermittent Metal GPU hangs (`kIOGPUCommandBufferCallbackErrorHang`); needs retry-with-CPU-fallback for production. Embedded Metal shader compilation costs ~7.5s on first binary launch (one-time per binary, not per session).**

**Spike #17.3 — whisper.cpp tuning sweep (FAIL — architectural, not parameter).** Five focused fixes applied: kLengthMs configurable, Silero threshold configurable per mic profile, special-token filter, `audio_sample_position_ms` field in TokenEvent, startup fixture verification. Smoke sweep tested kLengthMs values 1000, 800, 600ms. **All three FAIL C4.** Empirical measurements: kLengthMs=1000 → 453ms pods / 1322ms mac; kLengthMs=800 → 304ms / 280ms; kLengthMs=600 → 335ms / 308ms. No monotonic improvement with smaller ring buffer — variance dominates over scaling effect. **Root cause: whisper.cpp's `whisper_pcm_to_mel` ALWAYS computes a fixed-size mel spectrogram covering 30 × 16000 = 480000 samples (30 seconds), zero-padding shorter input to fill. The Transformer encoder ALWAYS runs on 1500 mel frames regardless of input audio length.** The C4 floor for whisper-small on Metal M3 is ~300-450ms for all kLengthMs values tested. **The #17.2 prediction (1s window → 120-160ms) was wrong because it assumed linear scaling with audio duration. Whisper's design uses a fixed 30s context — shorter input is zero-padded.** This learning generalizes across ALL Whisper-derived options: WhisperKit, faster-whisper, whisper-cpp-rs all run the same 30s-context Transformer encoder and will hit the same floor. Forward note: also caught a FluidAudio v0.14.7 VAD bug — `whisper_vad_segments_from_samples` rejects 160ms chunks as "too short" (applies `min_speech_duration_ms` filtering before returning); correct streaming usage is `whisper_vad_detect_speech` + inspect `whisper_vad_probs` directly.

### Decision: relax C4 budget to "whatever-best-possible," validate via 14-step smoke gate

Anton's product call: every general-purpose ASR engine designed before 2024 has an architectural latency floor above the original 200ms budget. We've validated this empirically across four spikes consuming ~3 weeks. The 200ms budget came from an aesthetic UX intuition ("widget feels instant"); empirical reality says the cheapest path to v1 is to accept the actual achievable latency and validate whether the UX still feels acceptable.

**Locked decision:** Ship whisper.cpp v1.8.4 + Silero VAD v5.1.2 + whisper-small (multilingual) at `kLengthMs=1000ms` with per-mic Silero threshold (0.5 pods, 0.3 mac). Special-token filter applied. `audio_sample_position_ms` field on TokenEvent. C4 measured 304-453ms (with cross-fixture variance). Run the 14-step smoke gate against real Voice Memos session, examine state-transition timing, decide ship/retreat based on perceived UX.

If the 14-step smoke gate produces unacceptable UX:
- Option A: Sherpa-ONNX with Moonshine — designed for streaming (sub-100ms target on CPU), no 30s context architecture, ONNX Runtime dependency
- Option B: relax to Apple SpeechAnalyzer for English + cross-platform engine for other languages (acceptance of two-engine architecture)
- Option C: relax further to ~600-800ms C4 and accept the trade-off

If the 14-step smoke gate produces acceptable UX:
- Implementation commit on `spike/13.5-hal-stop-settling` replaces AppleTranscriberBackend with WhisperCppBackend. M3.7 closes. Tag + merge to main.

### Lessons learned across the seven-week search

1. **Every off-the-shelf ASR engine has an architectural latency floor that does not appear in marketing benchmarks.** Apple SpeechAnalyzer = commit-boundary batching. Parakeet SlidingWindow = 15s batch. Parakeet EOU = ~2s cache warmup. Whisper = 30s mel context. The benchmarks report steady-state throughput, not first-token latency on cold or streaming inputs. **Future engine evaluations: start with first-token latency on a fresh inference call, not steady-state metrics.**

2. **NO-SKIPPING discipline is load-bearing for spike methodology.** Every spike except #16 hit early-abort conditions where pressing on would have wasted hours of measurement on a known-broken architecture. Spike #17.1.5 GATE 1 abort after 160ms smoke; Spike #17.2 EARLY-GATE STOP at 11.7× budget miss; Spike #17.3 SMOKE-GATE FAIL on first kLengthMs after the architectural insight surfaced. Each early stop produced cleaner evidence than running the full sweep would have.

3. **Smoke-then-gate pattern (single-fixture smoke before 10-fixture sweep) saved hours per spike.** When the smoke fixture revealed architectural mismatch (#17.1.5, #17.3), the full sweep was unnecessary. When the smoke fixture passed (#17.1, #17.2), the full sweep validated against multiple acoustic conditions.

4. **Side-by-side comparison reports are load-bearing evidence for product-owner decisions.** Anton's repeated question through the spikes ("is the smaller model worse?") required concrete token-text evidence, not just PASS/FAIL bits. The `tools/compare_to_17_*.py` scripts produce per-fixture markdown tables with first-N token text, latency deltas, confidence comparisons. This is the format that lets a product owner reason about accuracy-vs-latency tradeoffs.

5. **Fixture corpora are reusable across engine spikes if they cover the acoustic conditions you need.** The 10 .caf fixtures Anton recorded for Spike #16 (5 scenarios × 2 mics) carried through all four engine spikes via symlink. Adding `real_world_test.caf` (Anton's 30s continuous English) gave a real-world validation point. Manifest.csv with stopwatch-measured event times is the ground truth.

6. **Multi-gate spike structure with explicit STOP discipline prevents "good enough" verdicts on bad data.** Spike #17.2's gate structure (BUILD-GATE → SMOKE-GATE → EARLY-GATE → GATE 2) became the canonical template. Each gate has a single PASS/FAIL outcome and a clear next action. Soft verdicts ("seems okay, ship it?") are worse than hard FAILs.

7. **Architect-side reframings of evaluation criteria require explicit Anton-level approval, not just documentation.** Multiple times across the spike sequence, criteria reframings (C8 audio-time vs wall-clock, C9 RNNT confidence sentinel, C11 1GB memory budget for whisper-small) needed explicit approval because they materially change verdict logic. The pattern: reframe before Phase 2, never as a Phase 3 outcome.

8. **The `@main async` pattern is mandatory for Swift CLI scaffolds that bridge to async APIs.** Spike #17.1's first attempt deadlocked for 9.5 hours on `semaphore_wait_trap` because the CLI used `DispatchSemaphore` to bridge `await loadModel()` to synchronous `main`. The progressHandler callback dispatched back to MainActor (blocked on semaphore), creating a deadlock. `@main async` keeps the runloop alive and the cooperative thread pool servicing async work. Document for all future CLI spikes.

9. **Cached Core ML model directories from prior projects (TiltTalk on the same machine) can confuse FluidAudio's listing-phase validation on the new project.** Spike #17.1 hit a separate hang because TiltTalk's iOS-compiled Parakeet v3 cache existed at `~/Library/Application Support/FluidAudio/Models/parakeet-tdt-0.6b-v3/` and FluidAudio's macOS code path hung trying to validate it. Workaround: clean the cache before first run on a new platform.

10. **"Behavior correct under design A becomes wrong under design B" is a recurring failure mode requiring explicit re-evaluation.** Session 032 caught this with M3.7.3 fix #3's widget hide-on-token-silence (correct under disconnect-probe-reconnect, wrong under engine-always-warm). Spike #17.2 caught this with `kLengthMs` reduction (the prediction assumed linear scaling, but Whisper's 30s context invalidated the assumption). When the architecture changes, re-evaluate every prior assumption.

### Time

Session 033 wall-clock: **~35-50h** across multiple sub-sessions over three weeks. Breakdown estimate:
- M3.7 fix #6 + fix #7: ~8-10h (widget lifecycle, Bug B-prime, 7-state model)
- Spike #16 (RMS VAD): ~6-8h (fixture recording + algorithm + 27-combo sweep + REPORT)
- Spike #17.1 (Parakeet SlidingWindow): ~6-9h including the 9.5-hour semaphore-deadlock debugging detour (~3-4h productive after the hang)
- Spike #17.1.5 (Streaming EOU): ~3-4h (Gate 1 fast abort)
- Spike #17.2 (whisper.cpp baseline): ~6-9h (BUILD-GATE work, CMake + Metal, Swift-C bridging, Gate 1 sweep)
- Spike #17.3 (whisper.cpp tuning): ~3-4h (Gate 1 fast abort + architectural diagnosis)
- Session 033 doc consolidation: ~2h

**M3.7 cumulative across Sessions 026 + 027 + 028 + 029 + 030 + 031 + 032 + 033: ~60-83h actual against 3h estimate = ~20-28× estimate.** This is the worst estimate-vs-actual variance in the project. Driver: four-spike engine search produced four hard FAIL verdicts before identifying the path that meets the (relaxed) latency budget. The widget UX problem revealed at smoke gate (Session 032) cascaded into a fundamental engine question that no single spike could resolve.

**Architectural insight: the right answer here was not "find the engine that hits 200ms." The right answer was "validate that 200ms is the right budget." Anton's "relax to whatever-best-possible, then 14-step smoke" reframes the problem from an engine search into a UX validation, unlocking the cheapest path to v1.**

**Session 033 closes with documentation tarball delivered to Anton. The next session opens fresh with whisper.cpp integration as the active frontier.** The implementation commit will land on `spike/13.5-hal-stop-settling`, replacing AppleTranscriberBackend with WhisperCppBackend. M3.7 closes after 14-step smoke gate validates the UX. If smoke validates: tag `m3.7-complete`, merge to main. If smoke surfaces unacceptable UX: re-open with Sherpa-ONNX / two-engine / further budget relaxation per the decision tree above.

---


## Session 032 — 2026-05-18 — Third architectural rationalization-loop surfaced post-Session-031-smoke-gate: widget hide-on-token-silence (M3.7.3 fix #3 behavior, preserved by fix #5) violates locked spec "widget visible for entire mic-active session"; architect-product-owner dialogue locks full widget lifecycle (Phases A–G) — engine-ready signal = first buffer to SpeechAnalyzer, 2s silence threshold drives content-state changes NOT visibility, session-end linger = 3s full + 2s fade with hover-pause / hover-reset / in-place session-switch behavior; product spec + architecture doc updated with explicit widget-lifecycle section; fix #6 prompt deferred to fresh Claude session to avoid context-window degradation; tarball + closing-prompt-for-agent + opening-prompt-for-new-Claude bundled as Session 032 hand-off

**Format:** Short course-correction session. No code commits, no agent implementation work. Architect-side spec doc update + lifecycle design + hand-off prep. Triggered by Anton's catch of the widget-hide-on-silence behavior in the Session 031 smoke log that the architect had incorrectly classified as "expected fix #3 behavior."

### What happened

Session 031 closed with a claim that M3.7.3 was complete and a proposal to tag `m3.7-complete` + merge `spike/13.5-hal-stop-settling` to main. Anton held the tag/merge steps and re-read the smoke log carefully. **Identified a spec deviation in step 2 of the 4-scenario smoke gate that the architect had observed but mis-classified.** Specifically: `Panel removed: token-silence-fade / Token-silence timer fired — panel hidden after 4.000000s of silence` fires during the 20-second silence period within an active Voice Memo session. The widget hides; speech resumes 19 seconds later; widget reshows via `widget-reshow-timing: trigger=reshow`. This is an active-session widget-hide-then-reshow cycle.

Per the locked product spec ("widget stays visible for the entire mic-active session, even when no speech is detected"), this is a violation. The behavior is a holdover from M3.7.3 fix #3 widget-engine decoupling, which made architectural sense when the AudioEngine was being torn down on every inactivity probe (the widget hide mirrored the engine teardown as a UX signal). Under fix #5's engine-always-warm architecture, the rationale evaporates: the engine never tears down mid-session, so the widget should never hide mid-session.

**Architect-side miss classification:** In the Session 031 smoke gate analysis, the line `widget correctly hid via Panel removed: token-silence-fade after 4s of token-silence (M3.7.3 fix #3 behavior, expected)` was incorrect — that behavior is NOT expected per the spec; it was correct under fix #3's mental model but became a spec violation when fix #5 changed the architecture. The third rationalization-loop in the fix #5 arc, except this one slipped past the architect-side verbatim-code audit because the failure mode was "behavior that was correct under design A is wrong under design B" rather than "code doesn't match spec."

**Anton called the miss, raised the question.** Architect re-read the smoke log, confirmed the violation, and stopped the tag/merge plan. Session 032 opens with full widget-lifecycle design dialogue.

### Architect-product-owner dialogue: full widget lifecycle locked

Anton synthesized the canonical v1 widget lifecycle in 7 numbered items. Architect mapped each to current implementation state and identified three architectural decisions requiring product-owner input:

**Q1 — Widget appearance trigger.** "Once ready the widget appears" — does that mean engine-ready (first buffer to SpeechAnalyzer) or first-token-arrival (current fix #3 behavior)? **Anton chose engine-ready (Q1=a).** Tradeoff explicitly accepted: ~5–7s perceived delay when Zoom starts and user speaks immediately, because we don't want to show placeholder content that goes stale.

**Q2 — Silence-detection threshold for content-state change.** **Anton confirmed 2 seconds for v1.** May be refined to WPM-aware in M4.1. The silence-detection event is module-level (subscribable) so adding new consumers in future doesn't require architecture changes.

**Q3 — Session-switch-during-linger behavior.** When Zoom B starts during Zoom A's 3+2s linger window: **Anton confirmed in-place transition** — linger animation cancels when session B reaches engine-ready, content swaps from session-A-ending to session-B-active without disappearing. User perceives one continuous widget that changed content. Phase G in the locked lifecycle.

**Item 5 refinement.** Anton refined the linger pattern from "5 seconds linger" to "3s at full opacity + 2s fade-out" with hover behavior: hover during 3s pauses countdown (resumes on hover-end); hover during 2s fade cancels fade, snaps to 100%, restarts the full 3s. Click handler wired but no-op in v1 — v2 wires to session-stats view. The linger affordance exists in v1 so v2 composes cleanly without UX rework.

### Architectural decisions locked

1. **Widget appears at engine-ready, not at mic-on or first-token.** Engine-ready = first audio buffer flowing into SpeechAnalyzer. This is the load-bearing system signal. Visual silence during the ~5–7s SpeechAnalyzer cold-start is deliberate; the tradeoff is explicitly accepted by the product owner.

2. **Widget stays visible for the entire engine-ready portion of a session.** Content changes between "active" and "subtle" modes based on 2-second token-silence threshold; widget itself never hides mid-session. The M3.7.3 fix #3 4-second-token-silence hide timer is REMOVED in fix #6.

3. **Silence-detection is a module-level event, decoupled from widget visibility.** Other modules (M3.2 SpeakingActivityTracker for monologue reset, future WPM-aware content treatment in M4.1) subscribe to the silence-detection event. **Lock: silence-detection events trigger feature behavior, not widget visibility.** Architecturally modular so new consumers add without changing the visibility state machine.

4. **Session-end linger: 3s full opacity + 2s fade with hover-pause and hover-reset.** Hover during 3s pauses countdown; hover during 2s fade cancels fade, snaps to 100%, restarts the full 3s. Click handler wired but no-op in v1 (logs the click). v2 wires the click to session-stats view. This UX affordance exists in v1 specifically so v2's click handler composes without rework.

5. **Session switch during linger swaps content in-place (Phase G).** Linger animation cancels when new session reaches engine-ready. Widget snaps to 100% opacity if mid-fade. Content swaps from old-session-ending to new-session-active without disappear-reappear.

6. **The "behavior correct under design A becomes wrong under design B" failure mode is now a documented audit step.** Every architectural change must include an explicit pass to re-evaluate existing behaviors against the new architectural assumptions. Mitigation in the architect's pre-merge checklist: when an architecture changes, walk through every component that interacted with the old architecture and verify each behavior still makes sense.

### Procedural lessons

1. **Architect-side verbatim-code audit catches polarity-class bugs (Session 031 fix #5 caught two: polarity inversion + filter scope-creep) but does NOT catch "design-context-shift" bugs.** The hide-on-silence behavior was correct code; the bug was that the architectural context around the code had changed, making correct code into a spec violation. **Lesson: the pre-merge audit needs a second pass — "what behaviors were load-bearing under the prior architecture that are now redundant or wrong?"** Add to the architect-side close-out checklist.

2. **Three rationalization-class bugs in one architectural arc (Session 031's fix #5) is a strong signal that architectural reviews need explicit re-evaluation phases.** Polarity, scope-creep, and design-context-shift are all distinct failure modes. The fix #5 architectural change was substantial (engine-always-warm replaces disconnect-probe-reconnect) and the agent's mental model needed to update simultaneously across multiple files. The probability of catching all knock-on consequences in a single audit pass is bounded.

3. **The product owner catches what the architect misses.** Anton noticed the widget-hide behavior in the smoke log by reading the visual flow ("widget appeared then hid then reappeared") against the canonical spec he holds in his head. The architect was reading the smoke log against the implementation he knew, not the spec he had nominally accepted. **Lesson: product-owner-side review against the canonical spec is a load-bearing audit step, not a courtesy.**

4. **Three-deliverable session-handoff format works for "pause and resume cleanly" mid-milestone.** Session 032 produces: (a) closing prompt for current agent to verify clean repo state, (b) updated docs tarball with the full widget-lifecycle spec written down, (c) opening prompt for fresh Claude session to draft fix #6. The lifecycle decisions are captured in writing (spec doc + architecture doc + journal) so the next session doesn't depend on architect memory.

### Time

Session 032 wall-clock: **~1h** (architect-side spec design + product-owner dialogue + doc updates + hand-off prep). No agent implementation work in this session.

**M3.7 cumulative across Sessions 026 + 027 + 028 + 029 + 030 + 031 + 032: ~25–33h actual against 3h estimate = ~8–11x estimate, M3.7 STILL OPEN.** Driver of this session's contribution: ~1h to catch and design around a spec violation that would have shipped to main if not surfaced.

**Session 032 closes with three deliverables prepared for hand-off; fix #6 implementation deferred to Session 033 in a fresh Claude conversation.**
...
## Session 031 — 2026-05-17 — Spike #14 measures SpeechAnalyzer warm-up gap at 6.9s across AVAudioEngine teardown (PARTIAL — warm state cannot survive engine rebuild); Spike #15 confirms per-process HAL client enumeration via `kAudioHardwarePropertyProcessObjectList` + `kAudioProcessPropertyIsRunningInput` at ~50ms flip latency on macOS 26 (SUPPORTED); disconnect-probe-reconnect algorithm replaced wholesale by non-disruptive 1Hz polling; M3.7.3 fix #4 hard-reset (local-only); fix #5 ships engine-always-warm architecture; two architectural rationalization-loops caught pre-push (polarity inversion, `com.apple.*` prefix scope-creep); smoke gate reveals THIRD spec violation slipped through architect-side audit — widget hide-on-token-silence behavior from M3.7.3 fix #3 is incompatible with the locked spec "widget stays visible for entire mic-active session." M3.7 does NOT close — fix #6 pending in Session 032 to remove the hide timer and wire engine-ready trigger + linger sequence.

**Format:** Three-arc session — Spike #14 (warm-survival measurement, REJECTED hypothesis), Spike #15 (per-process HAL enumeration, SUPPORTED hypothesis with one constraint), and M3.7.3 fix #5 implementation + smoke gate. Two architectural failure modes caught pre-push by architect code-audit-before-merge discipline. A third failure mode (widget hide-on-silence) was observed in smoke but mis-classified as expected behavior; caught by product-owner-side spec audit at session close. M3.7 does not close at the end of this session; fix #6 deferred to Session 032 (architect-side spec design only) and Session 033 (fix #6 implementation in a fresh Claude conversation).

### What happened

Session opened with M3.7.3 fix #4 already on the branch as local-only commits from a prior session attempt — a 7-second warm-up discard filter applied on every probe-resume cycle. Smoke gate against Voice Memo (May 17 prior session) had proved fix #4 architecturally wrong: in a quiet room, the 7s discard window applied every 15s inactivity probe compounded into "deaf zones" where real speech was silently lost. "Let's go" at t=0.39s discarded. Continuous speech 11.35–30.25s largely lost across overlapping warm-up windows.

**Spike #14 — SpeechAnalyzer warm-state survival across AVAudioEngine teardown.** Standalone Swift package at `Spike/Spike14_AnalyzerWarmSurvival/`. Hypothesis: can SpeechTranscriber + SpeechAnalyzer + inputContinuation references be reused across AVAudioEngine teardown to avoid the warm-up cost? Empirical result: NO. Even keeping all three object references identical across engine rebuild, post-rebuild warm-up gap was **6.90 seconds**. Apple's framework resets analyzer state on audio source discontinuity. **Conclusion: the disconnect-probe-reconnect algorithm cannot avoid a multi-second cold start every probe cycle.** Verdict: PARTIAL. Production warm-up parameter would have been 7s — confirming the smoke-gate-failure root cause.

**Spike #15 — Per-process HAL client enumeration on macOS 26.** Standalone Swift package at `Spike/Spike15_HALClientEnumeration/`. Hypothesis: does macOS expose any API to determine whether processes OTHER THAN ITSELF are reading audio from a given Core Audio input device, WITHOUT requiring the calling app to stop its own engine first? Tested six candidate API categories. **Winner: `kAudioHardwarePropertyProcessObjectList` + `kAudioProcessPropertyIsRunningInput`.** Returns a list of `AudioObjectID` per audio process; each can be queried for PID + bundle ID + `IsRunningInput` Bool. **Manual smoke across three states (spike alone / +VoiceMemo recording / VoiceMemo stopped) showed clean flips: VoiceMemos PID=838 `IsRunningInput=false→true` within 50ms of record tap, `true→false` within 50ms of stop.** Production-grade signal latency. Verdict: SUPPORTED. **One constraint:** `com.apple.CoreSpeech` (PID stable, system dictation daemon) is permanently `IsRunningInput=true`. Must be filtered from the query as a baseline. Future macOS versions may add additional always-on daemons; baseline set is hardcoded for v1, extended as smoke surfaces new entries.

**Architectural implication of Spike #15:** the disconnect-probe-reconnect algorithm becomes obsolete. New design: AVAudioEngine and SpeechAnalyzer run for the entire session lifetime, warm up exactly once at session start, never re-warm. Probe cycles become a synchronous 1Hz query that does not touch the audio engine. Session lifetime is driven entirely by "is any non-baseline non-self process holding `IsRunningInput=true`?"

**M3.7.3 fix #4 hard-reset.** Two local-only commits (`b8b0b6f` and `8ab538e`) were destroyed via `git reset --hard 154d0fc`. Both had never reached origin, so no force push and no forensic loss. Branch returned to Spike #14 state. Fix #3 widget-engine decoupling work (`4fc955e`, `c328ae0`) was preserved — that work is sound.

**M3.7.3 fix #5 design.** Drafted as the architectural rewrite. Replaces `MicAvailabilityProber` protocol with `AudioProcessProber` returning `[pid_t]` of external readers. Replaces `inactivityTimer` + `runProbeCycle` + `resumeSession` + `finalizeAfterProbe` with `startPollTimer` — a `@MainActor Task` that polls every `probePollIntervalSeconds` (default 1.0). When `readers.isEmpty`, session ends with new `SessionEndReason.micFreedExternally`. AVAudioEngine and SpeechAnalyzer remain running for the entire session — teardown happens only at session end. M2.1 IRS listener stays as backup signal. M2.5/M2.7 widget+persistence work stays. Fix #3 widget-engine decoupling stays.

**Fix #5 implementation produced two architectural rationalization-loops caught pre-push.**

**Loop 1 — polarity inversion.** Agent reported the poll-loop as "if the result is non-empty calls endCurrentSession(reason: .micFreedExternally)." Architect read this and asked for the verbatim code body. Three snippets confirmed the agent's mental model: "TalkCoach yields when an external app wants the mic." Coherent narrative, wrong polarity. Design intent: empty readers → mic is free → end session. The agent had matched test expectations to the inverted logic, so the green test suite was misleading. **Architect drafted in-session correction prompt** (Change 1: invert conditional; Change 2: rewrite tests as three explicit polarity cases — ends-when-becomes-empty / continues-when-present / ends-when-empty-from-start). Agent shipped corrected commit `88b95bf`. All three new polarity tests pass.

**Loop 2 — `com.apple.*` prefix scope-creep.** After polarity correction, architect audited the production prober and found the baseline filter was implemented as `bundle.hasPrefix("com.apple.")` instead of the spec's `baselineBundleIDs: Set<String> = ["com.apple.CoreSpeech"]`. **The filter excluded Voice Memo (`com.apple.VoiceMemos`) — the primary smoke target.** Under the prefix filter, opening Voice Memo would trigger session-start via IRS listener, then the first poll would find Voice Memo filtered out as a "system daemon," readers=[], session ends within 1 second. Same failure mode as the polarity bug, different cause. Tests didn't catch it because only `com.apple.CoreSpeech` (excluded) and `com.zoom.us` (included) had explicit cases. **Architect drafted in-session correction** (Change 1: replace `hasPrefix` with hardcoded Set lookup; Change 2: add four new test cases asserting `com.apple.VoiceMemos`, `com.apple.FaceTime`, `com.apple.Music`, `com.apple.QuickTimePlayerX` all PASS `isExternalReader`). Agent shipped corrected commit `b892020`. All four new Apple-app inclusion tests pass.

**Final smoke gate.** Branch at `b892020`. Voice Memo + four-step scenario (per architect-Anton agreed protocol): start VM + "let's go" + 7s pause / speak 30s + 20s silence / speak 10s more / stop VM. Live log capture via `log stream --predicate 'subsystem == "com.talkcoach.app"' --info > ~/Desktop/fix5-smoke.log`.

**Results across all four steps:**

- **Step 1 (warm-up + brief utterance):** Session active 595CC43A. Tokens #1–9 progressively recognized "let / let' / let's / go", finalized as `'let's' t=[0.00–0.78] final=true` and `'go.' t=[0.78–1.20] final=true`. Widget appeared 57ms after first token. **"Let's go" captured at audio timestamp 0.00–1.20s — exactly when it was said. Architecturally impossible under fix #4 (warm-up filter would have discarded it).**
- **Step 2 (30s speech + 20s silence):** Continuous token flow #10 through #514 across 30 seconds of speech. **Zero `warm-up window started` log lines anywhere in the run** (fix #5 deleted that log line). **Zero `inactivity timeout fired`.** **Zero `probe cycle`.** Just heartbeat polls every second: `AudioProcessProber: 1 external reader(s) — session continues`. During the 20s silence, widget correctly hid via `Panel removed: token-silence-fade` after 4s (M3.7.3 fix #3 behavior, expected). 16 consecutive 1Hz polls during silence all reported "1 external reader(s) present" — session stayed alive throughout.
- **Step 3 (speech resumes after silence):** Token #515 arrived after ~19 seconds real-time delta. Audio timestamp `t=[32.40–53.89]` — SpeechAnalyzer had buffered the silence period internally and emitted the cumulative recognition span when speech resumed. **Widget reshow latency from first new token: 1.27ms.** Under any prior architecture, this would have been a 5-7s cold warm-up. The architectural test passed: engine stayed warm.
- **Step 4 (Voice Memo OFF):** Token #770 'okay.' final at t=[62.82–63.66]. One last `1 external reader(s)` poll. Then `AudioProcessProber: 0 external reader(s)` → `SessionCoordinator: poll — no external readers → ending session (.micFreedExternally)` → `Session ended: 595CC43A reason=mic-freed-externally duration 67.7s`. Within ~1 second of Voice Memo stop. **770 tokens transcribed across 67.7s at 11.4 tok/s average.**

**Smoke gate observed (with Session 032 correction):** The 4-scenario smoke gate ran as described. At session close the architect read the smoke as "all four pass" — incorrect. The widget-hide-on-token-silence behavior observed in step 2 was mis-classified as expected fix #3 behavior. Anton caught the miss at session-close audit; Session 032 captures the correction and locks fix #6 design. **M3.7 does not close at the end of this session.**

### Architectural decisions locked

1. **Rationalization-loop pattern locked as an architect-side audit risk.** When the agent describes a conditional as natural-language prose ("if X then Y"), the description can encode a coherent but inverted mental model that doesn't match the spec. Test suites match the inverted code and pass. **Mitigation: for any conditional that maps a domain state to an action ("when readers empty → end session", "when X is true → continue"), the architect requests verbatim code body and a verbatim test assertion before merge.** This caught two distinct failure modes in fix #5 alone, both pre-push. The audit cost is ~2 minutes per push; the alternative is a same-day fix-up commit and a wasted smoke run.

2. **Hardcoded spec values are sometimes hardcoded on purpose.** The spec called for `baselineBundleIDs: Set<String> = ["com.apple.CoreSpeech"]` — a single-entry Set bounded by Spike #15's empirical findings. The agent generalized to `bundle.hasPrefix("com.apple.")` ("more robust against future macOS daemons"). Generalization broke against currently-shipping Voice Memo. **Lock: when a spec value is bounded by spike evidence, the agent does not generalize it without explicit architect approval.** The spec value is the architectural commitment; the heuristic is the rationalization.

3. **The non-disruptive 1Hz polling architecture replaces disconnect-probe-reconnect.** Spike #14 proved the warm-state-survival path is closed (SpeechAnalyzer cannot survive engine teardown regardless of object retention). Spike #15 proved the per-process HAL query path is open. **Lock: M3.7.3 production architecture is engine-always-warm with a 1Hz HAL process-list poll.** AVAudioEngine and SpeechAnalyzer start once at session begin, run for the entire session, never teardown except at session end. The new `SessionEndReason.micFreedExternally` (raw `"mic-freed-externally"`) describes the trigger.

4. **The baseline daemon filter is hardcoded and grows by smoke discovery.** Initial set: `["com.apple.CoreSpeech"]`. Documented in source comment block: future always-on Apple daemons added when smoke surfaces them. Explicitly NOT prefix-matched. Voice Memo, FaceTime, Music, QuickTime under `com.apple.*` are correctly excluded from the baseline (they count as external readers). Test coverage asserts all four user-facing Apple apps PASS `isExternalReader`.

5. **Engine-warm session lifetime: AVAudioEngine teardown happens only at `endCurrentSession`.** Five entry points to session-end: `requestFinalize` (X-button), `micOffListener` (IRS=false, backup), `appSleep` / `appShutdown` / `appQuit` (system lifecycle), `micFreedExternally` (new — poll returned empty). All converge on the same `endCurrentSession()` path which stops the poll timer FIRST, then tears down AudioPipeline + AppleTranscriberBackend, then persists the session record.

6. **Settings deprecation pattern locked.** `inactivityThresholdSeconds` and `warmupWindowSeconds` settings persist in `UserDefaults` for backward-compat with prior install states, but are no longer surfaced in the Settings UI and no longer read into runtime behavior. New `probePollIntervalSeconds` setting (default 1.0, range 0.5–5.0) replaces them as the user-tunable parameter. **Lock: deprecated settings persist silently in UserDefaults but disappear from UI; new settings get explicit footer text describing what they control.**

7. **`logger.debug` for heartbeat events, `logger.info` for state-change events.** Poll heartbeat (`X external reader(s) present — session continues`) logs at `.debug` level so it doesn't fill `--info` log streams every second. Only state changes (`no external readers → ending session`) log at `.info`. Smoke gates with `--info` filter see only state-change events; verbose debug requires explicit `--debug` flag. Reduces signal-to-noise in smoke-gate log captures.

### Procedural lessons

1. **Two pre-push architect audits caught two distinct bugs that test suites missed.** Both bugs were polarity-class — the natural-language description of a conditional flipped the spec direction in a coherent-but-wrong way, and the corresponding test assertions matched the inverted code. The 312-green / 314-green / 318-green test counts looked clean at each fix #5 commit but the smoke gate would have failed instantly. **Lesson: green test suites are NOT a sufficient pre-merge audit when the failure mode is "design intent flipped by rationalization." The architect must request verbatim code body for conditional logic before push.** Two minutes of pre-push audit prevented two same-day fix-up commits and two wasted smoke runs.

2. **The NO-SKIPPING rule operated three times in this session.** (a) Fix #4 smoke failure → Spike #14 + Spike #15 in-session rather than accepting "good enough" on the broken filter. (b) Polarity bug caught and fixed in-session rather than punting to a follow-up. (c) Filter scope-creep caught and fixed in-session. **The rule's value compounds:** each round of in-session fix prevents a future round of "we found a bug from last session's work." Session 030's marathon discipline carried forward; Session 031's three NO-SKIPPING triggers had zero architect prompting to invoke.

3. **Live log capture via `log stream` to a file beats `log show --last Nm`.** The earlier smoke gate (fix #4) tried `log show --predicate ... --last 5m` after the smoke completed, and the 5-minute retention window had already expired — zero events visible. **Lock: smoke gate preamble starts `log stream --predicate '...' --info > ~/Desktop/foo.log` in a separate terminal BEFORE the smoke begins.** Captures everything regardless of when the analysis runs. Used successfully for fix #5 smoke; entire 67.7s session captured cleanly.

4. **Sub-agent reviews continue holding the credibility standard.** Fix #5 implementation went through Phase 3 self-review + sub-agent review on each of three commits (`a9fb9b1` initial, `88b95bf` polarity correction, `b892020` filter correction). Verbatim AC marker block disposed each time. Architect's verbatim-code request to the agent for the polarity audit produced exactly what was asked for (three labeled snippets in one fenced block), no narrative wrapping. **The reply-formatting discipline locked Session 027 + 030 held under stress (two pre-push corrections in one workflow).**

5. **Apple SpeechAnalyzer's progressive emission pattern is now documented behavior, not a bug.** During the 20s silence in smoke step 2 + 3, SpeechAnalyzer emitted no tokens — but when speech resumed in step 3, the first token arrived at audio timestamp `t=[32.40–53.89]`, a 21-second cumulative span. This is Apple's analyzer buffering the silence and emitting recognition as soon as the next speech event lands. **Lock: SpeechAnalyzer's token-emission cadence during silence is "none" — the framework batches everything until next speech and emits a cumulative-timestamp recognition.** M3.2 SpeakingActivityTracker will need to handle long t-span tokens that include silence periods — the token timestamps describe Apple's audio buffer span, not the user's actual speaking interval.

6. **Spike #14's NEGATIVE result was load-bearing.** Spike #14 was framed as "can SpeechAnalyzer warm state survive engine teardown?" The hypothesis would have unlocked a much-simpler architecture (keep one prober, just tear down audio engine). Result: NO, 6.9s gap regardless. **Without Spike #14, the architectural pivot to engine-always-warm via Spike #15 might have been deferred or scope-narrowed, since the prior algorithm was a working-but-broken middle ground.** The negative spike result was the evidence that pushed the team toward the better architecture rather than patching the prior one. NO-SKIPPING on Spike #14 (it could have been skipped — "we'll just do prober teardown anyway") prevented the team from settling at the local optimum.

### Time

Session 031 wall-clock: **~7h** (architect-side architecture + drafting, plus four agent implementation cycles, plus smoke gate, plus close-out). Broken down:

- Fix #4 architectural failure analysis + Spike #14 design + agent execution + report: ~1.5h
- Spike #15 design + agent execution + report + manual three-state smoke (architect + Anton coordinated): ~1.5h
- Fix #5 design (replacing disconnect-probe-reconnect wholesale) + drafting full prompt: ~1h
- Fix #5 implementation cycle + polarity audit + correction prompt + agent ship: ~1h
- Filter scope audit + correction prompt + agent ship: ~0.5h
- Smoke gate setup + 4-step run + log analysis: ~1h
- Close-out (this journal entry + backlog + ledger + tag + merge plan): ~0.5h

**M3.7 cumulative through Session 031: ~24–32h actual against 3h estimate = ~8–10x estimate. M3.7 does NOT close — fix #6 (widget lifecycle: engine-ready trigger + 3s+2s linger + content-state silence-detection) deferred to Session 033 per Session 032 architect-side spec design.** Driver: M3.7 was the project's first encounter with macOS HAL behavior at the application architecture level. Six Apple-framework runtime-discovery findings, two new spikes (#14, #15) opened mid-flight, an architectural pivot (disconnect-probe-reconnect → engine-always-warm) within the same milestone. **Spike #15's positive result was the unlock — once `kAudioHardwarePropertyProcessObjectList` was confirmed working, the architecture collapsed from "complex teardown-and-rebuild state machine" to "1Hz query + simple guard." The right architecture is small; getting there required the full exploration of the wrong ones. Session 032 caught the missing piece: the widget-visibility behavior preserved from fix #3 needed re-evaluation under the new architectural assumptions.**

**Session 031 start:** ~early afternoon local (Mexico, UTC-5).
**Session 031 end:** evening local.
**M3.7 does NOT close.** Fix #6 design captured in Session 032; fix #6 implementation deferred to Session 033 in a fresh Claude conversation.
