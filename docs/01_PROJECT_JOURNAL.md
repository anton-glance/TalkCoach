...
## Session 034 — 2026-05-21/22 — M3.7.4 whisper.cpp + Silero integration completed; engine-quality evaluation proves whisper-small architecturally wrong for filler detection; Round 1 verbatim tuning ships; token-silence flicker bug removed; engine decision pending Round 1/2 data

**Format:** Long single-day session (~5h+ active) dominated by infrastructure debugging, ending in a product-defining finding. The M3.7.4 production integration of whisper.cpp + Silero VAD landed and works end-to-end on AirPods, but a 2.5-min conversational accuracy test revealed whisper-small transcribes *intended meaning*, not *actual speech* — fatal for a filler-word-counting product. Round 1 whisper tuning (verbatim params + adjustable window + token-silence-flicker fix) shipped as `m3.7.4-r1-verbatim`. The verbatim sweep and engine decision are deferred to the next session.

### What happened

Session continued the M3.7.4 production integration (Architecture Z: single whisper.cpp v1.8.4 + Silero VAD v5.1.2 + whisper-small backend, replacing the retired Architecture Y dual-backend). Sub-commits 1–4.5 had already landed the PoC, the WhisperCppBackend actor, the additive VAD widget subscription, the single-backend collapse, and the VAD-debounce restoration. This session was supposed to be the 14-step smoke gate. It became, in order: a multi-hour infrastructure-bug fight, then the real engine-quality evaluation.

**Infrastructure bugs (ate most of the session).** Four distinct issues, all fixed:
1. **Model-path + log-redaction bug.** WhisperModelLoader used `Bundle.main.url(forResource:withExtension:)` without `subdirectory: "Models"`, so models in the bundle Models/ subfolder were never found → wiring nil → widget stuck warming. Compounded by os.Logger redacting all markers as `<private>`. Fixed atomically: subdirectory lookup + `privacy: .public` on widget-state and voice-active markers.
2. **Exit crash `GGML_ASSERT([rsets->data count] == 0)`.** whisper/Silero Metal contexts never freed at process exit. Fixed (`f8f7cbf`): WhisperCppBackend.shutdown() frees both contexts; AppDelegate.applicationWillTerminate bridges async shutdown via DispatchSemaphore+Task.detached+2s-timeout (FLAGGED as the Spike #17.1 deadlock anti-pattern — safe here because shutdown() doesn't re-dispatch to MainActor before signaling and the 2s cap prevents indefinite hang; TODO added for a MainActor-sync free path).
3. **Multi-hour test-host hang — ROOT CAUSE.** Production AppDelegate.applicationDidFinishLaunching booted the REAL backend (loaded the 487MB model, started a live mic session) INSIDE the XCTest host because the mic device was active. XCTest exits via `exit()` bypassing applicationWillTerminate, so singleton contexts were never freed → exit assertion → hang. Two consecutive "Run All Tests" runs hung for hours. Fixed (`3f0cd41`): guard at the very top of applicationDidFinishLaunching — `ProcessInfo.processInfo.environment["XCTestConfigurationFilePath"] != nil` → skip all production audio/engine/session/widget boot. After fix: 356/363 (1 skipped), clean exit, no hang. **The verification gate confirmed two engine-ready-named tests in the deletion list were scaffold-only and correctly KEPT, not deleted.**
4. **A1–A4 audio-path diagnostics.** Added at info level to localize the "no tokens" symptom. Revealed the built-in MacBook mic is signal-starved (RMS 0.002–0.015 during loud speech — matches Spike #16's far-field finding), while AirPods give healthy dynamic range (RMS 0.0000–0.2395). The engine was never broken; the built-in mic is just too quiet for reliable VAD/transcription. `input-device` log confirmed device 96 = "MacBook Air Microphone", inputVol=0.596.

**The engine-quality evaluation (the finding that matters).** Once the infrastructure was clean, Anton ran a deliberately-constructed 2.5-min conversational script (embedded fillers um/uh/like/you know/I mean/sort of, numbers as words and digits, self-corrections) on AirPods. 150s session, 328 tokens. The engine works end-to-end — real transcription, VAD-driven states, correct widget lifecycle (warming→counting→waiting→wrapping→fade). BUT the transcript proved **whisper-small transcribes intended meaning, not actual speech**: "three or four years"→"304 years", "2021 or 2022"→"2000 or 2020", "filler usage"→"fiddler usage", "let the pause"→"let the pulse", "pause"→"Paul", and most fillers dropped or converted to punctuation. whisper is trained on clean written-style transcripts and actively removes disfluencies — structurally wrong for a product whose entire job is counting and analyzing those disfluencies. Anton's reference Parakeet transcript (via WhisperKey, a Handy fork) on the same audio preserved fillers, numbers, and disfluent structure dramatically better.

**Reframe locked: whisper transcribes meaning; Parakeet transcribes speech. The product needs speech.** Anton's insistence on a conversational test over a "1-2-3 counting" test was vindicated — the counting test would have shipped a broken product.

**Reference-app investigation.** WhisperKey (github.com/levelup-app-factory/whisperkey, fork of cjpais/Handy) is the reference Parakeet app. Key findings: it is batch (press→speak→release→transcribe-whole-buffer, no LLM), which is why its output appears seconds after stopping and improves with full-utterance context — partly an engine advantage, partly a batch-vs-streaming advantage. Its engine layer is the Rust crate `transcribe-rs` (v0.2.8: whisper/parakeet/moonshine/sense_voice/gigaam), Silero VAD, rubato resampling. Parakeet V2/V3 are CPU-optimized ONNX/int8 (~473–478MB), no Metal. **This is a different, working Parakeet integration than the FluidAudio Swift path that Spike #17.1/#17.1.5 found too slow.**

**Windows-portability finding (tilts toward Parakeet).** whisper.cpp's GPU path on Mac is Metal (Apple-only); a Windows port means a per-platform GPU backend rebuild (CUDA/Vulkan/CPU). Parakeet via transcribe-rs is CPU-only → runs identically on macOS/Windows/Linux with no GPU code. WhisperKey itself ships cross-platform from one Rust+Tauri codebase precisely because of this. So Windows (planned next, ideally quick migration) is a second independent reason to take Parakeet seriously. Caveat: transcribe-rs Windows + real-time streaming in our architecture is unproven — a spike before commitment.

**Latency analysis (not a blocker).** For a live coaching widget, the measured ~1s token lag at 1s window is imperceptible. 3s/5s windows lag ~4s/~7s — fine for the accuracy *test*, not for production feel. Latency is not the constraint; accuracy is.

**Round 1 tuning shipped (`m3.7.4-r1-verbatim`, commit `d19db1b`).** Three deliverables, all behind UserDefaults (verbatim OFF by default, default config provably reproduces prior whisper behavior):
- **Verbatim params** via new `CWhisperVerbatimParams` struct + `cwhisper_full_verbatim` entry point (leaves `cwhisper_full` untouched): suppress_blank=false, suppress_nst=false, no_speech_thold=0.9 (RAISED — agent caught that the architect's prompt had the direction backwards; whisper semantics are `is_no_speech = (prob > thold)`, so raising keeps low-energy fillers), temperature=0.2, initial_prompt="Okay, um, so, like, you know, I mean, uh, sort of, right.", carry_initial_prompt=true. All seven field names confirmed against the vendored whisper.h (this whisper.cpp version uses `suppress_nst`, NOT the older `suppress_non_speech_tokens`).
- **Adjustable context window** (`whisperWindowMs`, default 1000) — the accumulator is linear (no ring buffer), so widening is a one-variable change; cross-chunk context comes only from initial_prompt.
- **Token-silence flicker bug REMOVED.** Two surviving call sites (SessionCoordinator relayTokens timer + FloatingPanelController $isInTokenSilence subscription) from incomplete sub-commit-4 removal caused the widget to flip counting→waiting twice mid-speech (whisper emits tokens in ~1s bursts; inter-burst gaps tripped the 2s token-silence timer while VAD correctly showed voice=true). Fully removed; widget .waiting now driven ONLY by the VAD path. grep "token-silence" in Sources/ returns zero.

**Other token-stream bugs identified (deferred, must fix before Phase 4):** duplicate/overlap re-emitted tokens (count went up by 4 for 3 words — overlap re-inference); `[BLANK_AUDIO]` NOT caught by the special-token filter (filter broken); silence hallucinations ("Sorry.", "Honestly.") counted; all per-token timestamps degenerate `t=[X.99–X.99]` (zero-duration, snapped to inference-window boundary — breaks Phase 4 WPM math).

### Product decisions locked (Anton's calls)

1. **V1 scope = real UI + WPM + monologue indicator.** No cuts. Monologue logic = Option (a): horizontal bar starts at 60s continuous talk, grows to max + signals at 90s, resets on 3s pause. Monologue indicator reverses its prior V2-deferred status — needs spec update (adds ~1–2 days). All numeric thresholds (60s/90s/3s, VAD silence/debounce, Silero per-mic thresholds, confidence filter, kLengthMs, linger durations, the new whisper verbatim params) must be exposed as adjustable App Settings for Anton to tune, then baked in.
2. **Model bundling:** ship models inside the .app bundle (Resources/Models/ via build phase from gitignored Vendor/models/). NO download UI. M3.7.5 download module DROPPED. Cold-launch Metal compile (~7.5s, first-launch-per-binary only) hidden behind onboarding screens (future M3.7.5 onboarding row).
3. **M3.7.4 widget/UX portion = DONE.** Widget lifecycle confirmed correct by Anton. The open item is engine quality, addressed by the Round 1/Round 2 sweep.
4. **Engine evaluation is data-driven, cheapest-first.** Round 1 = free knobs (verbatim params + context window) on whisper-small. Round 2 = whisper-medium (drop-in .bin swap; URL `https://blob.handy.computer/whisper-medium-q4_1.bin`), only if Round 1 doesn't recover fillers. If neither recovers fillers, the Parakeet/transcribe-rs path becomes the engine decision — with Windows portability as an added argument for it.

### Architectural / process decisions

1. **whisper-small is architecturally (not tunably) wrong for verbatim filler capture — pending confirmation by the Round 1/2 sweep.** The disfluency-removal bias is inherent to whisper's training across all sizes; larger models likely fix *acoustic* word errors (fiddler→filler) while still eating fillers. The sweep exists to confirm/deny this with data before an engine swap.
2. **Conversational test over counting test is mandatory for STT product validation.** The "1-2-3" test validates the pipe; only connected conversational speech with embedded disfluencies validates the product. Lock: STT engine acceptance requires a conversational-script accuracy pass, never a counting pass.
3. **XCTest-host production-boot guard is required for any app whose @main side effects touch live hardware.** Production launch must detect XCTest (`XCTestConfigurationFilePath`) and skip the live audio/engine/session boot. This was the root cause of multi-hour test hangs.
4. **PROPOSED CONVENTION (for next-session ratification): agent must show verbatim diff or commit SHA for any "fixed X" claim before the architect approves moving on.** The exit-crash was diagnosed and instructed two turns before it actually landed; the gap cost hours of hung test runs. A "show the SHA/diff before we proceed" gate closes it.
5. **Second TDD-discipline violation in project history** (M2.6 was first): agent shipped a no-debounce passthrough AND skipped the red-phase debounce tests in sub-commit 4.5. Caught and corrected. Reinforces the failing-tests-committed-and-not-modified lock.

### Known repo-state issues (cleanup deferred)

- **Commit `d19db1b` is bloated:** 816 files / ~1M insertions. Pulled in `Spike/Spike17.2_WhisperCppVAD/whisper.cpp` as an EMBEDDED git repo (a fresh clone won't contain its contents), both model `.bin` files (487MB + 864KB), all Spike 16 sweep CSVs, and research screenshots. gitignore debt — clean before more commits stack on top. Fix candidates: gitignore the model blobs and embedded whisper.cpp, or convert whisper.cpp to a submodule.
- **Public remote (`origin/main`) is stale at Session 028 / Architecture Y.** None of the Architecture-Z M3.7.x work is pushed; the spike branch `spike/13.5-hal-stop-settling` is local-only. Canonical state is the local working tree. Backlog/journal in the remote still describe the retired dual-backend architecture.

### Time accounting

- Session end: 2026-05-22 03:18 UTC (2026-05-21 22:18 EST local).
- Wall-clock: long single-day session, ~5h+ active (infrastructure debugging dominated; engine evaluation + Round 1 plan/implement/ship in the final stretch).
- Breakdown: ~60% infrastructure bug-fighting (model path, log redaction, exit crash, test-host boot, A1–A4 diagnostics), ~25% engine-quality evaluation + reference-app investigation, ~15% Round 1 plan + implement + commit.
- Variance: M3.7.4 has run well over nominal; the integration is functionally complete but the engine-quality finding reopens the engine question. Estimate guidance: **OS-API + ML-integration modules with live-hardware side effects run 2x+ due to test-host and lifecycle traps; budget a test-host-isolation step up front.**

### Next session

1. **Run the verbatim sweep** (4 passes, same 2.5-min script, AirPods): baseline → verbatim-1s → verbatim-3s → verbatim-5s. Quant filler-retention + word-fidelity + hallucination-cost comparison.
2. **Round 2 if needed:** whisper-medium drop-in.
3. **Engine decision:** whisper-tuned vs Parakeet/transcribe-rs (Windows portability favors Parakeet).
4. **Deferred token-stream fixes (before Phase 4):** dedup, [BLANK_AUDIO] filter repair, degenerate timestamps.
5. **Repo cleanup:** gitignore model blobs + embedded whisper.cpp; consider submodule.
6. **Spec/backlog:** monologue-indicator reversal from V2; M3.7.5 onboarding row; settings-adjustable-parameters audit.
7. **Ratify** the "show SHA/diff before proceeding" convention into `00_COLLABORATION_INSTRUCTIONS.md`.
