...
## Project totals (current state, end of Session 024)

- Calendar days from project start (May 1) to current: **8 days**
- Approximate total wall-clock: **~55–60h** (Phase 0 spikes ~30h + Phase 1+2 modules ~17h + Spike #4 Phase 2 ~3h + M3.1 ~3.5h + M3.4 ~2.5–3h)
- Tagged modules complete: 13 (M1.1, M1.2, M1.3, M1.4, M1.5, M1.6, M2.1, M2.3, M2.5, M2.6, M2.7, M3.1, M3.4) + Spike #4 Phase 2. **M3.1 + M3.4 are code-complete with smoke deferred to M3.7.**
- Modules remaining for v1: ~9 (Phase 3 modules ex-M3.1/M3.4 + Phase 4 + Phase 5 + Phase 6)

---

## Session 034 — 2026-05-21/22 (UTC-5)

- Start: (continued from prior session, same day)
- End: 2026-05-22 03:18 UTC / 2026-05-21 22:18 EST
- Wall-clock: ~5h+ active
- Module: M3.7.4 (whisper.cpp + Silero production integration)
- Breakdown: ~60% infrastructure debugging (model path, log redaction, exit crash, XCTest-host boot guard, A1–A4 diagnostics); ~25% engine-quality evaluation + WhisperKey/transcribe-rs reference investigation; ~15% Round 1 verbatim tuning (plan + implement + commit `d19db1b`, tag `m3.7.4-r1-verbatim`).
- Variance: M3.7.4 well over nominal. Driver: live-hardware @main side effects booting inside the XCTest host caused multi-hour test hangs; ML+OS-API integration lifecycle traps.
- Guidance: OS-API + ML-integration modules with live-hardware side effects run 2x+; budget a test-host-isolation step up front.
