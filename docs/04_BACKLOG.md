...
> **⚠️ Session 034 status (2026-05-22):** This Phase-3 table predates Architecture Z and is STALE. Architecture Y (Apple SpeechAnalyzer + Parakeet dual backend, M3.5a/b, M3.6 download flow, M3.8 swap) is RETIRED. Current architecture is a single whisper.cpp v1.8.4 + Silero VAD backend (Architecture Z). M3.7.4 (production whisper.cpp/Silero integration) is functionally COMPLETE and tagged `m3.7.4-r1-verbatim`; the open item is ENGINE QUALITY (whisper-small drops fillers — see Session 034 journal). Round 1 verbatim tuning shipped; verbatim sweep + engine decision (whisper-tuned vs Parakeet/transcribe-rs) pending next session. M3.7.5 model-download module DROPPED (models bundled in .app). Monologue indicator reversed from V2 into V1 scope (needs spec update). **The Session 034 journal entry is authoritative for current Phase-3 state; this table will be reconciled in a dedicated doc-surgery pass.**
