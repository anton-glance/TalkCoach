...
| M3.7.6 | VAD master-clock gate infrastructure (Silero) — raw gated transitions, M4-ready | 📋 READY TO SCOPE (Spike #19 PASS; architecture LOCKED Session 037 addendum). **M3 builds the COMPLETE tested gate/infra; M4 builds WPM+Monologue on top with NO surprises.** Gate = Silero v5 (production `ort`/ONNX bridge) running PARALLEL to Parakeet on the same audio stream → minimal jitter-debounce (~100-200ms, signal-cleanliness only, NOT a product grace) → emits RAW gated transitions (`speechStarted`/`speechStopped`) to any subscriber. Gate does NOT apply product pause-tolerances — WPM's 2s grace and Monologue's 2.5s reset are METRIC rules applied DOWNSTREAM in M4 to the same raw stream. **M3 close criterion (no-surprises proof):** synthetic pauses 150ms/1s/2.2s/3s through the real gate → assert clean transitions (chatter-debounced only) → TWO stub subscribers (WPM-stub@2s, Monologue-stub@2.5s) each derive correctly from the ONE shared stream. Phase-1 plan to be written against the real M3.4 production audio code (RollingAudioWindow/tap/buffer flow) next session. Includes 2s-grace Settings control (now an M4/WPM concern) + AppKit layout-recursion investigation. | est. TBD post-scope | Spike #19, M3.7.4 |
