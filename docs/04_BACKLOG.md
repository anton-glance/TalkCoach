...
| M3.7.4 | whisper.cpp v1.8.4 + Silero VAD integration into production transcriber stack | 🔄 NEXT (Session 034 target). Replaces `AppleTranscriberBackend` with `WhisperCppBackend` on `spike/13.5-hal-stop-settling`. Configuration: whisper-small multilingual (ggml-small.bin, ~487MB resident), Silero VAD v5.1.2 (~2MB ONNX), kLengthMs=1000, per-mic Silero threshold (0.5 pods / 0.3 mac), special-token filter, audio_sample_position_ms field on TokenEvent. CWhisper C bridging shim with module.modulemap (multi-line format). CMake build with `-DGGML_METAL=ON -DGGML_METAL_EMBED_LIBRARY=ON -DBUILD_SHARED_LIBS=OFF`. Metal command-buffer retry-with-CPU-fallback wrapper. Widget state machine subscribes to Silero VAD signal (NOT tokenStream) for speaking/listening transitions. 14-step smoke gate against real Voice Memos session, with state-transition CSV pivot timing validation — load-bearing UX decision point. **If UX unacceptable:** escalate to Sherpa-ONNX + Moonshine (designed for sub-100ms streaming), or two-engine architecture (Apple SpeechAnalyzer for English + cross-platform for other languages), or further C4 budget relaxation. **If UX acceptable:** tag `m3.7-complete`, merge `spike/13.5-hal-stop-settling` to main. | 8-12h estimate (CMake + Metal + Swift-C bridging + state-machine rewire + 14-step smoke gate) | M3.7.3, Spike #17.2, Spike #17.3 |
