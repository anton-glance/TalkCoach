...
| M6.8 | **Audio-engine device-switch robustness — STILL OPEN. 12+ iterations failed Session 050. Path B locked.** Switching the mic input mid-session causes one of: session-restart cascades (3+ sessions in <1s), stuck `warming` indefinitely, stuck dashes, 70+ second teardowns piling up, or pulsing-logo that never recovers. Baseline (`m5.7-complete`) and all 12 M6.8 iterations exhibit some variant of this — confirmed Session 050 smoke. **Spike #19 (Session 050) proved the architectural root cause:** the AVAudioEngine tap's underlying AsyncStream is finished by the OS-level device teardown, which cascades through `AudioBufferBroadcaster.drive`'s `for await buffer in source` loop, fires `.finish()` on every consumer continuation, and kills Parakeet's `bufferTask` regardless of any suppression-flag patching downstream. See `docs/spikes/spike-19-broadcaster-pause-architectural-test.md`. **Path B (locked for Session 051):** insert an intermediate session-lifetime AsyncStream (`mediumStream`) between the AVAudioEngine tap and the broadcaster, with a pump task that copies per-engine buffers into it. When the device changes: cancel pump, recreate engine, restart pump into the SAME `mediumStream`. Broadcaster never sees the source change; consumers see a brief gap. Visible artifact: widget transitions `counting → warming → counting` for ~1-2s. Session ID + counters preserved. Full spec in `docs/session-051-bootstrap-pathB.md`. **What's rejected from the 12 iterations:** banned symbols `isInRewire`, `rewireWarmupUntil`, `applyWPMVoiced`, `onRewireStateChanged`, `rewireInProgress`, `rewireAudioPipeline`, `replaceAudioProvider`; the "100% invisible" UX goal; the iterate-suppression-flag pattern. **What's retained as wisdom:** `provider.recreate()`, `waitForHWBoundInputFormat` polling, `needsEngineRecreateOnNextStart` flag, 300ms HAL settle, `MicMonitor.deviceChangeUntil` 500ms window, `MicMonitorDelegate.isSwitching` flag. **Hard limits for Session 051:** 3-iteration ceiling, no new suppression flags, single architectural change. **Fallback if Path B fails:** Option A (ship with documented limitation "session ends on mic switch, re-activate to resume", ~2h) — product call by Anton, NOT another architect iteration. | Original: 4–6h. **Session 050 spent ~11h with zero shipped progress.** Path B estimate: additional 4–6h focused. | M5.x, S4, Spike #19 |
...
**Phase 6 total: ~22–30h** (was ~22–24h; Session 050 spent ~11h on M6.8 with zero net progress, +4–6h Path B remaining for Session 051; if Path B fails, Option A fallback ~2h).
