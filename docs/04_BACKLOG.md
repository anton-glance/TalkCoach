...
| M5.4a | `WPMCalculator` STOPS publishing when 2s silence-hold fires (fixes declining-WPM-during-silence bug — visible in S044/S045 smoke as `wpm-refresh A_median=-1` on VAD-stop). **PLUS counting-trigger threshold (confirmed needed S045 smoke):** widget currently enters `.counting` on a single short word ("ok"/"well"/"right" → `warming→counting reason=token`) before real WPM is ready, producing a gray-dashes window the M5.4 opacity-hold cannot mask because the trigger itself is premature. Needs a minimum-utterance gate so counting commits only when meaningful speech is detected, not on a one-word transient. Algorithm change; M4.1-saga safety work required. Live multi-session smoke before tag. **Plus waiting→data no-flash fade-in:** when real data arrives while in `.waiting` (50% dashes), the widget must fade straight from the dashed/dim waiting presentation to numbers+colors WITHOUT first flashing a full-opacity gray-dashes content frame (S045 smoke #5: "100% gray dashes then numbers"). M5.4 added the window-alpha hold (`skipOpacityChange`); this is the remaining CONTENT-layer concern, observable once the counting-trigger gate stops premature `.counting` entry. | 📋 | 2–3h | M5.4 |
| M5.5 | Liquid Glass material + Reduce Transparency fallback + hover (scale/lift only, NO color change). Drag-to-move folded in (exists from M2.5). **Hover-reveal of close (X) button:** X is hidden by default, appears only on mouse hover over the widget; never visible during cold-start/counting/waiting/idle. **NOTE: X POSITION already fixed in M5.4** (pinned top-trailing in all states) — M5.5 only adds the hover-gated visibility, the corner placement is done. | 📋 | 4h | M5.3 |
