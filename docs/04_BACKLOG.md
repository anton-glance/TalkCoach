...
| M3.7 | Token stream wiring: audio → engine → unified token output | 🔄 IN PROGRESS (Sessions 026 + 027 + 028 + 029 + 030 + 031 + 032, ~25–33h actual against 3h estimate = ~8–11x). First speech tokens emitted Session 026. **Spike #13 CLOSED Session 030** — all four formal probe paths (A KVO-AVCaptureDevice, B aggregate-AVAudioEngine, C AVCaptureSession-via-CMIO, D cascade-SCK-HALscan-CMIOExtension) FAILED; six locked Apple-framework runtime-discovery findings establish no standard signal distinguishes our capture from external at HAL level. Session 030 architect pivot to disconnect-probe-reconnect algorithm; Spike #13.5 measured HAL stop-settling time at max 45ms. **M3.7.3 Session 031: Spike #14 REJECTED (SpeechAnalyzer warm-state cannot survive AVAudioEngine teardown, 6.9s gap); Spike #15 SUPPORTED (per-process HAL client enumeration via `kAudioHardwarePropertyProcessObjectList` + `kAudioProcessPropertyIsRunningInput` at ~50ms latency on macOS 26). Architectural pivot from disconnect-probe-reconnect to engine-always-warm + 1Hz HAL polling. Fix #5 ships engine-always-warm at branch HEAD `b892020`.** **Session 032: smoke gate post-audit by Anton identifies the M3.7.3 fix #3 4-second-token-silence widget hide is incompatible with the locked spec "widget stays visible for entire mic-active session" — fix #6 design locked (engine-ready trigger, 3s+2s linger with hover behavior, in-place Phase G session-switch, silence-detection event decoupled from visibility); fix #6 implementation deferred to Session 033 in fresh Claude conversation.** Tags NOT created. Branch NOT merged. M3.7 closes only after fix #6 smoke clean. | 3h estimate + Spike #13 (CLOSED) + Spike #14 (CLOSED) + Spike #15 (CLOSED) + M3.7.3 (~8h actual through Session 032) | M3.1, M3.5 |
| M3.7.3 | Engine-always-warm session lifecycle + 1Hz HAL process-list polling + `probePollIntervalSeconds` user setting + widget lifecycle (engine-ready trigger + 3s+2s linger + content-state silence-detection) | 🔄 IN PROGRESS (Sessions 031 + 032, ~8h actual through Session 032; fix #6 implementation deferred to Session 033). Fix #5 shipped engine-always-warm + 1Hz HAL polling: AVAudioEngine + SpeechAnalyzer run for entire session lifetime; warm up exactly once at session start; never re-warm. `SystemAudioProcessProber` queries `kAudioHardwarePropertyProcessObjectList` + `kAudioProcessPropertyIsRunningInput`, filters out `getpid()` and hardcoded `baselineBundleIDs = ["com.apple.CoreSpeech"]`. When poll returns empty, session ends with new `SessionEndReason.micFreedExternally`. M2.1 IRS listener stays as backup signal; X-button + sleep + quit + shutdown paths preserved. Old `inactivityTimer` / `runProbeCycle` / `resumeSession` / `MicAvailabilityProber` deleted; `inactivityThresholdSeconds` and `warmupWindowSeconds` settings persist in UserDefaults for backward-compat but removed from UI. **Fix #6 pending (Session 033):** remove M3.7.3 fix #3 token-silence hide timer; rewire widget-appear to engine-ready signal (first buffer to SpeechAnalyzer); implement 3s full + 2s fade linger sequence with hover-pause-during-full and hover-reset-during-fade; add silence-detection as a module-level event decoupled from widget visibility; implement Phase G in-place session-switch when new mic-on signal arrives during linger. Smoke gate criteria for fix #6: 4-scenario Voice Memo test where widget stays visible through the full 20s silence period in step 2 (no `Panel removed: token-silence-fade` log during active session). **Two pre-push architectural rationalization-loops caught by architect verbatim-code audit in fix #5:** polarity inversion (commit `88b95bf` corrected) and `com.apple.*` prefix scope-creep (commit `b892020` corrected). **Third architectural failure mode caught by product-owner spec audit in Session 032:** "behavior correct under design A becomes wrong under design B" — fix #3 widget-hide was correct under disconnect-probe-reconnect architecture (mirrored engine teardown) but a spec violation under fix #5's engine-always-warm. | 6–10h | M3.7, M2.7, Spike #14, Spike #15 |
