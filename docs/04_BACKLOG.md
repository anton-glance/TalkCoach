...
| M3.7.6 | Real-time VAD (Silero) as pipeline master-clock + metric gating | 📋 READY TO SCOPE (Spike #19 PASS, Session 037 — ungated). Voice-activity is the TRIGGER gating the ENTIRE metric pipeline — voice present → count (WPM/monologue/fillers); voice stops → 2s grace → declare silence → all metrics PAUSE. Silero v5 VALIDATED (onset ~32ms, zero-FP far-field, no mid-sentence fragmentation). Build = wire the validated signal in as the gate (de-risked: model/ONNX-path/real-time-pipeline all proven in spike). **Silero via the production `ort`/ONNX Rust bridge** (M3.7.4's, statically linked). Cross-platform (ports to Windows w/ Parakeet). **2s grace is a SINGLE dual-purpose debounce** — bridges inter-word VAD dips (metrics run through a sentence) AND defines end-of-turn. **Includes:** 2s-grace Settings control; metric-gating wiring; AppKit layout-recursion warning investigation (carried from M3.7.4). To be scoped fresh (not session-tail-rushed). | est. TBD post-scope | Spike #19, M3.7.4 |
