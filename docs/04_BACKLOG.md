...
| M3.7 | Token stream wiring: audio → engine → unified token output | 🟡 PIPELINE COMPLETE / M3.7.3 PENDING (Sessions 026 + 027 + 028 + 029 + 030, ~17–22h actual). First speech tokens emitted Session 026. **Spike #13 CLOSED Session 030** — all four formal probe paths (A KVO-AVCaptureDevice, B aggregate-AVAudioEngine, C AVCaptureSession-via-CMIO, D cascade-SCK-HALscan-CMIOExtension) FAILED; six locked Apple-framework runtime-discovery findings establish no standard signal distinguishes our capture from external at HAL level. **Architect pivoted to disconnect-probe-reconnect algorithm** (Session 030 product-owner call); Spike #13.5 measured HAL stop-settling time at max 45ms with tight distribution. **M3.7.3 implements the algorithm Session 031.** | 3h + Spike #13 (CLOSED) + M3.7.3 (~6–8h estimated incl. settings parameter + smoke gate) | M3.1, M3.5 |
| M3.7.3 | Disconnect-probe-reconnect algorithm + inactivity-threshold-as-settings-parameter | 📋 NEXT SESSION (031). Implements Anton's architectural pattern (locked Session 030): when inactivity timer fires, save session to SwiftData (M2.7 path), tear down AudioPipeline, wait ~100ms for HAL state to settle (per Spike #13.5 measurement), read IRS in our absence; if false finalize session, if true rebuild AudioPipeline and resume capture into the same session record. **Inactivity threshold migrates to user-facing Settings parameter** (was hardcoded constant in M3.7.2). Default value picked during implementation (likely 15–30s). M3.7.2 inactivity-timer scaffolding kept as defense-in-depth fallback. Algorithm composes shipping code (M2.1 listener pattern, M2.7 SwiftData, M3.7.2 inactivity-timer trigger) with no new APIs, no new permissions, no new framework dependencies. Smoke gate includes real Voice Memos / Zoom test + AirPods test (HR3 coverage). | 6–8h | M3.7, M2.7, M3.7.2 |
| M3.8 | Mid-session language re-detection / swap (if Spike #2 mechanism supports it) | 📋 | 4h | M3.4, M3.5 |

**Phase 3 total (EN-first sprint scope, M3.7 pipeline complete + Spike #13 CLOSED + M3.7.3 pending): ~31–33h with M3.5b + M3.6 deferred to post-WPM-milestone.**
