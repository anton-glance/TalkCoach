...
| M3.7.6 | Real-time VAD (Silero) as pipeline master-clock + metric gating | 📋 NEW (Session 036, product reframe). Voice-activity is the TRIGGER gating the ENTIRE metric pipeline — voice present → count (WPM/monologue/fillers); voice stops → 2s grace (Settings, default 2s) → declare silence → all metrics PAUSE so the gap doesn't pollute counts. Onset instant, offset 2s-graced. **Silero VAD via the EXISTING `ort`/ONNX Rust bridge** (M3.7.4 built+statically-linked it — a second ONNX model on existing plumbing, not a new subsystem). Cross-platform-mandated (ports to Windows alongside Parakeet; Apple-native VAD disqualified as macOS-only). Replaces the structurally-~10-14s window-only dim latency with sub-second detection decoupled from the 10s WPM window. **Includes:** 2s-grace Settings control; metric-gating wiring; investigate the AppKit layout-recursion warning seen at M3.7.4 teardown. **GATED behind Spike #19.** | est. TBD post-spike | Spike #19, M3.7.4 |
| Spike #19 | Silero VAD far-field validation through existing ONNX bridge | 📋 NEW (Session 036). VALIDATION spike, not a bake-off (candidate decided by cross-platform constraint → Silero). Question: does Silero, via the existing ONNX/Rust bridge, detect voice on the BUILT-IN FAR-FIELD MacBook mic with acceptable latency + accuracy? Pass bar inherits Spike #16 far-field lesson: work at sitting distance, REJECT HVAC/keyboard/fan (false-positive = counts silence as speech), CATCH soft conversational speech (miss = pauses mid-sentence). Measure onset latency, offset latency, and the two failure modes SEPARATELY. Gating wiring is M3.7.6 build, NOT the spike — spike validates the SIGNAL only. | est. 3-4h | M3.7.4 |
