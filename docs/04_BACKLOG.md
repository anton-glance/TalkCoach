...
| ID | Module | Status | Estimate | Depends on |
|---|---|---|---|---|
| M4.1 | `WPMCalculator` on the gate stream: sliding 10s-window average, updated every 3s (setting `wpmRefreshInterval`), pauses beyond `wpmPauseThreshold` excluded from the denominator (FM2 — pauses must not crash WPM). Subscribes to `VADTransitionEvent` + token word-counts. | 📋 | 4h | M3.7.6, M3.7.4 |
| M4.2 | `MonologueDetector` on the gate stream: continuous-talk timer for N min (setting `monologueTargetMinutes`); any pause > `monologuePauseThreshold` zeros the streak. Reset-rule validation gated behind Spike #11. | 📋 | 4h | M3.7.6, Spike #11 |
| M4.3 | Wire the widget fade to `wpmPauseThreshold` (replace M3.7.6's TEMPORARY 2s `static let` UI hold with the real setting-driven signal). | 📋 | 1h | M4.1 |
...
**M4 transcription-quality items (from S038 live smoke):**
- Investigate whether `parakeet-rs` can force English-only decoding. Parakeet TDT v3 is multilingual; `LanguageDetector` picks en_US but the model can emit other-language tokens on garbled first-hop audio (saw a Russian fragment on hop 1). Would corrupt WPM word-counts if not constrained.
- WPM should discard or down-weight the first (cold-start) hop — its tokens are low-quality regardless of language.
