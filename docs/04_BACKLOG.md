...
| M6.8 | **Audio-engine device-switch robustness — ✅ COMPLETE Session 051, tagged `m6.8-complete` at `dba303a`.** Mid-session device switches (built-in ↔ AirPods NC including HFP 24kHz transitions ↔ AirPods Pro) preserve session ID, counters, locale, Parakeet `bufferTask`, Silero VAD gate. Widget transitions `counting → warming → counting` for 2–7s during the swap. Architecture (Path B, locked Session 050 via Spike #19, shipped Session 051): session-lifetime `mediumStream` + per-engine `engineStream` + pump task bridging them, so the broadcaster and all consumers never see the source finish on engine swap. `switchDevice()` is the single rebuild authority — `AudioPipeline.recover()` and the in-pipeline `AVAudioEngineConfigurationChange` observer removed; observer reinstalled in `SessionCoordinator` and routed through `micDeviceChanged()`. Multiple-event device-change sequences (AirPods Pro HFP negotiation fires 2-3 events per physical action; config-change-during-switch) are handled by cancel-and-supersede in `micDeviceChanged()`. Tap installed with HW-stabilized format via install-then-swap pattern; falls back to `format: nil` on poll timeout. `RollingAudioWindow` auto-rebuilds its cached `AVAudioConverter` on input-format change. Smoke validated 9 clean switches in 3 minutes across all common AirPods modes. Banned-symbol grep clean. 645 tests, 644 pass, 1 skip, 0 fail. **V1 limitation tracked as M6.8b.** | Actual: ~14-15h total across Sessions 041–051 (~11h S050 + ~3-4h S051) vs original 4–6h estimate = ~3x. Path B itself ran on-estimate within S051. | M5.x, S4, Spike #19 |
| M6.8b | **Cumulative HAL state corruption hangs MainActor under sustained rapid mic-switch churn (v1.x).** Session 051 stress smoke: after 9 successful sequential switches across 3 minutes between built-in / AirPods NC / AirPods Pro, the 10th switch hung. Forensics: one of the synchronous AVAudioEngine calls (`removeTap`/`stop`/`installTap`/`prepare`/`start`) is blocked inside CoreAudio targeting a now-destroyed audio object ID. MainActor is frozen. Final log line before silence: `AudioObjectSetPropertyData: no object with given ID 265`. Widget shows last-rendered frame (`---`). `AudioProcessProber` polling also stops (it's MainActor-bound). User recovery: Locto menubar → Quit → reopen (~3s). Not addressable from Swift code: `Task.cancel()` cannot preempt a synchronous CoreAudio kernel call; a Task.sleep watchdog needs MainActor to fire its recovery body, which is exactly what's frozen. **V1.x investigation directions:** (a) periodic preventive engine-recreate every N switches to flush HAL state before corruption accumulates; (b) `Thread`-based hard watchdog with process-level supervisor that can force-restart the audio plane via spawning a fresh helper process; (c) reproduce conditions in isolation to file an actionable Apple Feedback rdar. **Real-world impact: very low.** Threshold is ~8+ rapid switches in a few minutes; typical 30–60min call has 1–3 switches. Document in v1 release notes as known edge case. | 4-6h investigation + fix in v1.x | M6.8 |
...
**Phase 6 total: ~14h remaining** (M6.8 ✅ shipped Session 051 at ~14–15h actual; M6.8b ~4–6h deferred to v1.x; M6.1–M6.7 + M6.9–M6.10 = ~14h remaining).
