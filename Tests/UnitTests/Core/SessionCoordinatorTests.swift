...
final class SessionCoordinatorTests: XCTestCase {
    ...
    // MARK: - AC1: micActivated with coachingEnabled starts session
    ...
    // MARK: - AC2: micActivated with coachingDisabled does not start session
    ...
    // MARK: - AC3: micDeactivated during active session ends session
    ...
    // MARK: - AC4: micDeactivated while idle is no-op
    ...
    // MARK: - AC5: sequential sessions have unique IDs
    ...
    // MARK: - AC6: pause mid-session terminates active session
    ...
    // MARK: - Resume while mic still active does NOT start new session
    ...
    func testResumeWhileMicStillActiveDoesNotStartNewSession() async {
        ...
        // Resume — mic still active, but MicMonitor won't re-fire micActivated
        settingsStore.coachingEnabled = true
        await Task.yield()
        XCTAssertEqual(sut.state, .idle, "No new session should start on resume alone")
    }

    // MARK: - Rapid mic toggles create multiple distinct sessions
    ...
    // MARK: - AC8: start initializes MicMonitor
    ...
    // MARK: - AC8: stop tears down MicMonitor and ends active session
    ...
    // MARK: - AC9: consumer notifications on MainActor
    ...
    // MARK: - AC7 / Convention 4: coachingEnabled live-sync

    func testCoachingEnabledLiveSync() async {
        makeSUT(coachingEnabled: false, isRunning: false)
        sut.start()

        // Mic activates while coaching disabled — no session
        fake.stubbedIsRunning = true
        fake.simulateIsRunningChange()
        await Task.yield()
        XCTAssertEqual(sut.state, .idle)

        // Deactivate mic first
        fake.stubbedIsRunning = false
        fake.simulateIsRunningChange()
        await Task.yield()

        // Enable coaching via SettingsStore (simulating @AppStorage menu-bar write;
        // UserDefaults → SettingsStore live-sync is verified in SettingsStoreTests)
        settingsStore.coachingEnabled = true

        // Mic activates — now session should start
        fake.stubbedIsRunning = true
        fake.simulateIsRunningChange()
        await Task.yield()
