...
// ---- CSV parsing ---------------------------------------------------------------
// Columns: 0=update_index, 1=emission_ms, 2=gap, 3=audio_sample_position_ms,
...
private func parseCSVRow(_ line: String) -> TokenRow? {
    ...
    guard let idx    = Int(parts[0]),
          ...
          let audioMs = Int(parts[3]),
          let conf   = Float(parts[6]) else { return nil }
    let isConf = parts[5] == "1" || parts[5].lowercased() == "true"
    return TokenRow(
        updateIndex: idx, emissionMs: eMs, gapFromPreviousMs: gMs,
        audioSamplePositionMs: audioMs,
        text: parts[4], isConfirmed: isConf, confidence: conf
    )
}
...
func printTable(_ results: [CriterionResult]) {
    ...
    print("\(pad("ID",4))  \(pad("Name",40))  \(pad("Verdict",10))  \(pad("Measured",22))  Budget")
    print(String(repeating: "-", count: 108))
    for r in results {
        print("\(pad(r.id,4))  \(pad(r.name,40))  \(pad(r.disposition.rawValue,10))  \(pad(r.measured,22))  \(r.budget)")
    }
}
...
let kLengthMs   = Int(flagValue("--k-length-ms", args: args) ?? "1000") ?? 1000
...
let fixtures = [
    ...
    "real_world_test",  // present in #17.3 (corrected symlink)
]
...
let rules = budgetRulesFor(kLengthMs: kLengthMs)
print("\n[verdict] kLengthMs=\(kLengthMs), strictMode=\(rules.strictMode)")
if rules.strictMode {
    print("[verdict] Strict rules applied: C6 budget ≤\(rules.c6GhostTokenBudget), C9 multiplier \(rules.c9ThresholdMultiplier)")
}
