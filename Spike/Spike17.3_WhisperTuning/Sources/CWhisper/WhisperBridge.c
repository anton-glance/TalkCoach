...
// whisper_vad_segments_from_samples applies segment-level filters (min_speech_duration_ms,
// min_silence_duration_ms) that reject short per-chunk inputs (160ms < any minimum duration).
// Instead: call whisper_vad_detect_speech to populate internal probs, then compare the
// maximum probability in the result against the custom threshold directly.
...
bool cwhisper_vad_detect_speech_threshold(
    ...
    // Run the model (populates internal probability array regardless of threshold)
    whisper_vad_detect_speech((struct whisper_vad_context *)ctx, samples, n_samples);

    // Inspect raw probabilities
