...
func buildClipResult(fixture: String, rows: [CSVRow]) -> ClipResult {
    ...
    // Inter-confirmed-update gaps (for latency percentile).
    var confirmedGaps: [Double] = []
    if let firstConfirmed = confirmedRows.first {
        confirmedGaps.append(firstConfirmed.emissionMs)  // gap from session start
    }
    for i in confirmedRows.indices.dropFirst() {
        confirmedGaps.append(confirmedRows[i].gapFromPreviousMs)
    }
