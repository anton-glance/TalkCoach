...
**Current state (Session 039 close):** **M4.1 (WPM metric) COMPLETE — tagged `m4.1-complete`.** The widget shows a single live WPM number (EMA-smoothed, `wpmEmaAlpha` default 0.70), working across UNLIMITED sessions per launch. WPM = raw per-hop `word_count × 6`, NO cross-hop accumulation, floor `minWordsForReading=3` + `minVoicedSecondsForReading=2.0`. Reaching this required resolving a six-bug multi-session lifecycle cascade (VAD-guard, VADGate `finish()`, RollingAudioWindow buffer reset, **ParakeetBackend engine keep-alive** — ORT rc.12 can't reinit OrtSessions in-process, so create once + reuse — and `hopStream`/`tokenStream` per-session recreation) plus a deterministic widget linger-race crash (cancel-linger-restore-warming). Spike #20 (Parakeet/ORT multi-session) folded in. 445 tests (444 pass, 1 skip). Engine LOCKED since Session 034 (Spike #18): Parakeet TDT v3 int8 via `parakeet-rs` (Rust/ONNX/CPU), 10s rolling window / 3s hop, Architecture AA. VAD master-clock LOCKED since Session 038 (Spike #19): Silero v5. **Phase 3 DONE.** **Active frontier:** M4.2 — `MonologueDetector` on the gate stream. Live branch `spike/13.5-hal-stop-settling` (main is stale). Known follow-ups: SwiftLint not on agent PATH (run manually); `rw: append` logging → `.debug`; settings housekeeping + dead language control removal; parked widget polish (waiting→visible 2s debounce, 1s-refresh feel). See `01_PROJECT_JOURNAL.md` Session 039 and `04_BACKLOG.md` Phase 4.
