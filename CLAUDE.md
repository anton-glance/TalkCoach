...
**Current state (Session 051 close):** **Phases 3, 4, 5 DONE; in Phase 6 (polish & ship).** Last shipped: **M6.8 (AirPods/HFP mid-session device-switch robustness) COMPLETE — tagged `m6.8-complete`** via Path B (session-lifetime `mediumStream` survives engine recreation; tap bound to HW-stabilized format; rapid events serialized via cancel-and-supersede). 645 tests (644 pass, 1 skip). The full runtime pipeline is wired end-to-end: mic → AudioPipeline → ParakeetBackend → WPMCalculator + MonologueDetector → widget (live WPM + monologue tint/L3 pulse, no mocks). Engine LOCKED since Session 034 (Spike #18): Parakeet TDT v3 int8 via `parakeet-rs` (Rust/ONNX/CPU), 10s rolling window / 3s hop, Architecture AA. VAD master-clock LOCKED since Session 038 (Spike #19): Silero v5. WPM = raw per-hop `word_count × 6` (no cross-hop accumulation), EMA-smoothed (`wpmEmaAlpha` 0.70), floors `minWordsForReading=3` + `minVoicedSecondsForReading=2.0`. **Planned remaining v1 order:** M6.11 (onboarding impl — currently design-handoff-only at HEAD `eafa35e`) → M6.12 (Locto rebrand) → M6.7 (notarized DMG). **Known ship gaps:** model download (M3.7.5) not yet implemented — models must be placed manually until then; mic permission only requested from a `#if DEBUG` menu; session metrics persist as zero stubs (M4.6/M4.7, no v1 UI reads them). **Deferred v1.x:** M6.8b (HAL hang after ~9 rapid switches), M6.9, M6.10. Other follow-ups: SwiftLint not on agent PATH (run manually); dead language-control cleanup (`LanguagePickerView`, `toggleLocale`). See `01_PROJECT_JOURNAL.md` Session 051 and `04_BACKLOG.md` Phase 6.
...
│   ├── Speech/                        ← ParakeetBackend, ParakeetModelLoader, SileroVADGate, SileroModelLoader, RollingAudioWindow, TokenConsumer
│   ├── Language/                       ← LanguageDetector, detection strategies, WhisperLIDProvider, ScriptCategory (LID for Latin↔CJK)
│   ├── Analyzer/                      ← SpeakingActivityTracker, WPMCalculator, MonologueDetector, EMASmoother
